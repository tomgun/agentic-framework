...
**Quick option (use your existing token):**
...
This extracts the token from macOS Keychain and makes it available as an environment variable. The devcontainer picks it up via `${localEnv:GH_TOKEN}` and passes it into the container. `run.sh` does this automatically.
