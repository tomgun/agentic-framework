...
- When `auto`: proceeds without review
...
| **Multi-Agent / Git Worktrees** (F-0031-0033, F-0097) | Tools exist, protocol documented | Never been used in a real multi-agent scenario. `worktree.sh` exists. AGENTS_ACTIVE.md template exists. F-0108 helper scripts still planned (TODO). | MEDIUM — Needs real-world validation. The coordination protocol is thorough on paper. |
| **Plan-Review Loop** (F-0120) | Partially active | `ag plan` exists. Plan-review is configurable. But agents often skip directly to implementation without going through the loop. `ag implement` has a plan-review gate but it's not consistently triggered. | HIGH — The mechanism is complete. Need stronger behavioral enforcement or a structural gate that blocks `ag implement` without an APPROVED plan artifact. |
