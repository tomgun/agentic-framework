...
9. **Enforcement chains must fail-closed under blocking mode** — every link in a hook → gate → shell chain must deny on error when `state_enforcement: blocking`. A Python crash (exit 1) or hook timeout that silently allows the action converts a blocking gate into a permission. Use enforcement-level-aware error handling: fail-closed for blocking, fail-open for non-blocking. Check enforcement level with fast mechanisms (grep on STACK.md) that don't share the failure mode of the gate itself. See `FRAMEWORK_DEVELOPMENT.md` § "Fail-open error handling is incompatible with blocking enforcement"
10. **Silence is permission to an LLM agent** — when a gate fails silently (no output, no error), the agent interprets absence of friction as compliance. Unlike human developers who might notice something odd, LLM agents take the path of least resistance. If you intend to block, you must produce visible output. Defense-in-depth layers should both block AND warn — a warning-only layer at least surfaces in conversation even if the blocking layer failed
