...
# Create FEATURES.md with F-001 in_progress
...
send_prompt "F-001 User Authentication is complete - all acceptance criteria pass and all tests are green. Please mark it as shipped."

# Verify agent behavior
FAILURES=0

# Agent should reference F-001 and shipped status
check_output_contains "F-001\|feature\|status\|shipped" \
    "Agent references F-001 and shipped status" || ((FAILURES++))
