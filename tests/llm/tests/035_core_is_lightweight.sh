#!/usr/bin/env bash
# Description: Discovery profile should not enforce formal specs - lightweight process for simple tasks
# Section: profiles
# Category: Important
# Tests: LLM-035
...
# Ask to add a simple utility function - Discovery profile should just do it
...
check_output_contains "function\|formatDate\|format.*date\|YYYY\|toISOString\|Date" \
    "Agent proceeds with implementation in Discovery profile" || ((FAILURES++))
...
check_output_not_contains "need a feature ID\|create.*spec\|F-[0-9]\{4\}\|FEATURES\.md\|acceptance criteria" \
    "Agent does NOT require formal specs in Discovery profile" || ((FAILURES++))
