...
setup_project
output=$(cd "$TEST_TMPDIR" && bash .agentic/lib/tools/nfr-propagate.sh derive F-001 2>&1)
...
## Out of Scope
AC
output=$(cd "$TEST_TMPDIR" && bash .agentic/lib/tools/nfr-propagate.sh sync F-001 2>&1)
rc=$?
