...
setup_project() {
  ...
  # Minimal STACK.md
  cat > "$TMPDIR/.agentic/STACK.md" << 'STACK'
## Settings
profile: discovery
STACK
  # Minimal paths.sh and settings.sh stubs
  cat > "$TMPDIR/.agentic/lib/paths.sh" << 'PATHS'
PROJECT_ROOT="${PROJECT_ROOT:-$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)}"
ROOT_DIR="$PROJECT_ROOT"
export ROOT_DIR PROJECT_ROOT
PATHS
  cp "$PROJECT_ROOT/.agentic/lib/settings.sh" "$TMPDIR/.agentic/lib/settings.sh" 2>/dev/null || \
  cat > "$TMPDIR/.agentic/lib/settings.sh" << 'SETTINGS'
get_setting() { echo "${2:-}"; }
get_profile() { echo "discovery"; }
is_formal_like() { return 1; }
SETTINGS
}
