...
- `ag hooks register` / `ag hooks install` also wires the Claude Code statusline (`project | branch | model | ctx % | N% 5h, M% 7d - reset HH:MM, Day HH:MM TZ (updated at main agent response) | task`). Project name comes from `git config remote.origin.url` so it's correct inside Docker (where the toplevel basename is the mount point, e.g. `workspace`). Model uses the envelope's `display_name` (falls back to `id`). The rate-limit cluster is zero-config — straight from Claude Code's `rate_limits` envelope, in local time with one shared timezone label. The dim `(updated at main agent response)` trailer anchors that values are a snapshot from the last main-agent turn; `/usage` triggers a refresh.
