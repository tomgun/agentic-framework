...
- `ag hooks register` / `ag hooks install` also wires the Claude Code statusline (`project | branch | ctx % | 5h % reset HH:MM | wk % reset Day HH:MM | task`); 5h/wk segments only show when `quota_pro_max_window_tokens` / `quota_pro_max_weekly_tokens` are set in STACK.md
