...
| Plan created / planning complete | STOP -> Save plan as DRAFT. If `plan_review_enabled: yes`: auto-continue — dispatch Critic + Advocate tasks in parallel (fresh context), synthesize. If `convergence: auto`, auto-approve on convergence; if `manual`, present to user. Then `ag implement F-XXXX` (creates WIP). |
