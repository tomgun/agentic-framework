...
5. **Project kickoff & decomposition** — during brownfield spec generation (`ag specs`) or greenfield project setup, the system naturally produces a hierarchy: epics → features → tasks. The unified backlog should support this from day one — decomposition outputs a dependency-aware work plan, not a flat list. Epics decompose into features with parent links and ordering; features can have sub-tasks. This replaces the current pattern where `ag decompose` produces child features but can't express cross-feature dependencies or mixed-granularity work items in the backlog.
6. **Backlog as index, not detail store** — the backlog itself is a lightweight scheduling index: ID, type, summary, priority, dependencies, status, and links. All rich detail lives in dedicated files that already exist (plans in `journal/plans/`, acceptance criteria in `spec/acceptance/`, specs in `spec/FEATURES.md` or `spec/PLANNED.md`). A planned feature can accumulate full plans, AC drafts, and design notes before implementation — the backlog entry just links to them. This keeps the backlog scannable (token-efficient for agents, readable for humans) while supporting arbitrarily rich pre-implementation detail per item.
7. **Safe completion — no silent data loss** — `ag backlog done` currently pops position 0 blindly. If something else was implemented (out-of-order work, reprioritization mid-session), the wrong item disappears with no trace. Fix: (a) `done` must take an explicit item ID (`ag backlog done F-XXXX`), not assume position 0; (b) completed items move to a "done" section or archive rather than being deleted; (c) if the completed item wasn't at position 0, warn but proceed (don't silently discard the wrong thing). No information should be lost from the backlog — items transition states, they don't vanish.
