...
**User insight**: Tomas designed a five-phase quality improvement epic grounded in three specific pain points affecting user projects: (1) NFR constraints living in a separate section that agents treat as paperwork, (2) spec-analyze.sh being advisory-only so agents implement against vague ACs, (3) no mechanism to verify tests actually exercise ACs. The key architectural decisions: (a) **shared AC parser as Phase 0** — consolidating four divergent regex implementations before adding enforcement, recognizing that inconsistent parsing undermines any quality gate; (b) **flat-list specs keep 80% threshold** — after the dialectical review revealed that 95% of existing specs lack priority group headings, Tomas chose non-regressive rollout over a policy cliff, with priority groups as opt-in stricter enforcement; (c) **NFR constraints as P1 ACs inside Acceptance Criteria** — moving NFRs inline so existing enforcement applies automatically, eliminating the separate-section-as-paperwork failure mode; (d) **`--gate` mode with default exit 0 preserved** — backward-compatible extension that doesn't break existing CI users. The plan went through 4 dialectical review iterations, each addressing substantive concerns: fallback for flat-list specs, `--skip-clarity` flag namespace, re-implement skip mechanism, mixed-format behavior, and gate placement sequencing.
