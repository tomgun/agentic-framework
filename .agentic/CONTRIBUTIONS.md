...
### T-0023 — "Don't make the agent re-read 115 lines; emit targeted patches" (2026-05-14)

**User insight — Reduce behavioral burden by giving agents pre-computed deltas, not raw artifacts**: The original TODO framing was "memory-check.sh should: (1) fix worktree bug, (2) when stale, generate a precise diff showing what changed, (3) output structured instructions the LLM can apply as targeted patches." That third point is the load-bearing one — it inverts the usual advisory-script pattern. Most checks tell the agent *what's wrong* and leave reconciliation as a behavioral exercise ("re-read the file and update memory"). The user's reframe: *the script prepares the changes, the LLM merges them.* That collapses the agent's job from understanding-then-applying down to applying, which removes both a context window full of reading and a class of "agent read but didn't reconcile" failures. Adopted directly: `memory-diff.sh` now emits `PATCH N/N — ADD|REMOVE|MODIFY section "..."` blocks whose `-`/`+` lines map straight onto `Edit` calls. The pattern generalizes — any advisory check that today says "look at X and figure it out" is a candidate to be reshaped into "here are the N specific edits to apply."

**User confirmation that latent in-scope bugs belong in the same fix**: Mid-planning I surfaced two bugs that weren't in T-0023's text (wrong SEED_FILE path made the script a complete no-op; missing version marker made the stale check unreachable). User said "scope ok" — a validated judgment call that improvements/enforcement/hardening of an existing tool belong in one commit, not a series of cosmetic follow-ups. This matches the no-feature-inflation rule and avoids the failure mode where the headline feature ships on top of broken preconditions and nobody notices for weeks.

### V5 ARCHITECTURAL DECISION — User Reframed the Framework's Center of Gravity (2026-04-25 → 2026-04-26)

This entry covers the architectural insights that *justified* V5 itself, upstream of Sprint 1. They span four threads — real-world project failures, market research and M.A.'s autonomous workflow, the leaked Claude Code architecture paper, and the transform-vs-greenfield decision. All four predate Sprint 1 and are the highest-leverage contributions in the redesign cycle. Sources: `docs/research/2026-04-25-swarm-orchestration-and-close-out-hardening.md`, `docs/research/2026-04-26_deep_dive_claude_architecture_2604.14228v1.pdf`, `.agentic/journal/plans/2026-04-26-framework-ground-up-redesign-plan.md`, `.agentic/journal/plans/2026-04-26-agentic-af-2-greenfield-path.md`.

#### Thread 1 — Real-world project failure as empirical foundation

**User insight 1 — Downstream-session failure analysis**: User observed a systemic failure in a downstream Claude Code project — a single session ran 2 days, shipped 12 `feat:` commits, and never invoked any of the framework's close-out paths (`ag commit`, `ag done`, `ag backlog done`, `ag verify`). User did the transcript analysis personally: identified that resume signals like "continue" / "go on" weren't in the `completing-work` skill's trigger list, and that `on-bash-merge-detect.sh` exists but `on-bash-commit-detect.sh` does not. This single observation became the founding empirical case for V5 — proof that single-agent self-enforcement has a structural ceiling, not just an implementation gap. (sibling doc line 30)

**User insight 2 — 55% firefighting tax as the metric that matters**: User surfaced the commit-ratio inversion — `chore(state)` + `fix(...)` now make up 55% of commits, vs feature work being the majority three months prior. Adopted as evidence stream #3 in the V5 plan and as the operational metric for whether the redesign is working. The framework verification meta-loop (R-531) is justified specifically against this number. (V5 plan line 23, line 366)

**User insight 3 — "Wishlist is half the product, not nice-to-haves"**: User reframed the unbuilt items in TODO.md / proposed ADRs / future-tense JOURNAL entries / Theme A–J catalog as *the half of the product that didn't get built because firefighting consumed the runway* — not a backlog of optional enhancements. This shaped the AF2 greenfield doc's four co-authoritative capability sources (PRINCIPLES + CAPABILITY_SPEC + FEATURES + Wishlist) and forced V5 to ship `ag preview`, `ag deploy`, regulatory compliance, localization, crash reporting, etc. as Phase 5 deliverables rather than perennial backlog dust.

**User insight 4 — "We are not building this framework to build this framework"** (Mar 11, restated as V5 framing): User repeatedly grounded scope decisions in the framework-serves-user-projects principle. Translated in V5 to the explicit measurement question: "does this help teams using the framework ship better software?" not "does this clean up framework internals?" Shaped the Phase 5 production-gap themes (deploy / compliance / localization / crash reporting) as first-class V5 deliverables.

#### Thread 2 — Market research and M.A.'s autonomous workflow

**User insight 5 — Initiating the M.A. autonomy audit**: User asked directly whether M.A.'s claimed multi-hour autonomous swarm workflow ("tuntikausia autonomisesti") is real, and what 2026 SOTA actually looks like — with explicit suspicion that the framework's enforcement leaks too easily for that style of workflow to be reliable. The audit (4 rounds of agent peer review) resolved: the workflow is real and productized in Claude Code Agent Teams (4/2026), `metaswarm` v0.11.0, Cursor 2.0, GitHub Copilot Agent, Google Jules, OpenAI Codex Web, Anthropic Managed Agents — no longer cutting-edge speculation. (sibling doc line 30, 38)

**User insight 6 — Calibrated SOTA framing**: User's framing of M.A.'s "deterministic → adaptive → autonomous" as oversimplified became V5's three-tier capability source: 2026 SOTA is **spec-driven AND adaptive AND verified**, not one or the other. This drives V5's Tier 0 (deterministic git-layer) + Tier 1 (voluntary intelligence) + Tier 2 (fresh-context critic) + Tier 3 (Agent Teams) layering. (sibling doc line 258)

**User insight 7 — Calibrated skepticism, undercredited primitives**: The audit's verdict — *"He's not lying. The workflow is real. He's selectively narrating the parts that work and abstracting away the engineering (sandbox, state, hard gates, escalation). Your skepticism was calibrated to your *visible* enforcement (CLAUDE.md rules, advisory hooks, skills) — but your framework already has structural primitives (gates.py, sentinel files, PreToolUse deny) that you weren't crediting."* — directly shaped the V5 plan's "what doesn't work / what does work" division. (sibling doc line 296)

**User insight 8 — OW framework competitive analysis as design catalyst** (Apr 2, expanded): User initiated the comparison with the OW framework that surfaced hook-based intelligence injection. In V5, this lineage extended into the explicit market-research grounding: every architectural move in V5 maps to a productized 2026 competitor (Agent Teams = Tier 3, Managed Agents = sandbox, metaswarm = orchestrator/worker, opencode-swarm = declared file scope, Cursor 2.0 = path-scoped rules).

#### Thread 3 — Claude architecture research doc translation

**User insight 9 — 1.6%/98.4% as center-of-gravity test**: User absorbed the leaked Claude Code architecture paper (`2026-04-26_deep_dive_claude_architecture_2604.14228v1.pdf`, 46pp, VILA Lab MBZUAI) and translated its headline finding — ~1.6% of the codebase is decision logic, 98.4% is operational harness — into the framework's V5 inversion. Anthropic itself does not try to constrain the model with rules; they invest in deterministic infrastructure *around* the model. The framework had been doing the opposite for months. (V5 plan line 17)

**User insight 10 — "Stop fighting the model's training"**: User's translation of the Claude architecture finding — that single-agent enforcement is structurally bounded by post-training (OpenAI Instruction Hierarchy, Anthropic agentic-misalignment) — became V5's organizing thesis. The fix is *not* more rules in the agent's context; it's separating execution from acceptance, narrowing worker scope, and pushing critical enforcement out of any single session. (V5 plan line 31)

**User insight 11 — Adopted PDF §2.2 13 principles wholesale**: User ratified adopting the 13 Claude Code principles (deny-first, graduated trust, defense in depth, externalized programmable policy, context as scarce resource, append-only durable state, **minimal scaffolding / maximal harness**, values over rules, composable multi-mechanism extensibility, reversibility-weighted risk, transparent file-based config, isolated subagent boundaries, graceful recovery) as V5's revised design principle layer. The "minimal scaffolding / maximal harness" principle — direct from Anthropic's own design — is the single sentence that justifies Tier 0 + JSONL spine over more in-session prompting. (V5 plan §"Design principles (revised)")

**User insight 12 — Promotion rule executed en masse**: User invoked PRINCIPLES.md's own promotion rule (*"If a behavioral rule has been skipped 3+ times across sessions, promote it to a higher enforcement level. Repeated failures are evidence of misclassification, not insufficient documentation."*) as the warrant for re-tightening every toned-down principle in V5: D2 → Tier 0 git-layer; D4 → Tier 0 TDD; R2 → Tier 2 critic-gated; F3 → Tier 1 voluntary intelligence with Anatomy hook; D7 → honest "state portable, enforcement Claude-first." This executes the framework's own self-documented rule that v0.7x rarely actually executed.

#### Thread 4 — Transform-in-place vs. starting fresh from scratch

**User insight 13 — Chose transform over greenfield AF2**: At the 2026-04-26 architectural decision point, with both paths fully blueprinted (V5 transform plan + AF2 greenfield doc, ~840 lines), user chose transform-in-place. Rationale: greenfield's only durable advantage is "clean mental model from day 1" — and that doesn't justify discarding install-base, the 21 quality_knowledge files, contracts.py, the 41 YAML contracts already built, F-041's 235 intelligence-engine tests, and the skills marketplace (F-008) just shipped. AF2 saves ~8–12 weeks but loses everything else. Transform delivers the same V5 architecture across 32–44 weeks while preserving accumulated value. (greenfield doc line 4-13)

**User insight 14 — Reversibility criteria for switching back**: User defined the explicit conditions under which the transform decision should be revisited mid-flight: (a) Phase 0/1 reveals operational-harness simplification (Phase 3) is more painful than expected; (b) Anthropic Agent Teams API takes a hard incompatible turn; (c) cross-tool D7 abstraction layer surfaces hidden cost; (d) a new spec-format insight makes legacy migration painful; (e) v0.7x user-facing pain accumulates faster than transform delivers fixes. The transform plan is reversible at every phase boundary; Phases 0–2 deliverables (hooks, TUI, critic dispatch) are reusable in either path. (greenfield doc line 371-381)

**User insight 15 — V5 sprint pattern: consolidate into a single feature branch**: When given the option to branch per R-NNN item or use one sprint branch, user chose consolidation — adopted as the V5 sprint pattern beyond Sprint 1. Rationale: multiple experimental items advance together, review happens at the boundary, history stays clean. Sprint 1 covered R-007 + R-001 + R-002 + R-008 in `feat/v5-sprint-1`. Future Phase 0 fan-out waves use the same shape. (Already credited in Sprint 1 entry; lifted here as the durable pattern.)

**User insight 16 — Local-first, remote-optional as deployment philosophy** (Sprint 1, lifted to durable principle): User established that pure-local development is a first-class V5 path, not a degraded mode. GitHub/cloud is belt-and-suspenders. Shapes every V5 component: R-002 honors gate semantics on raw `git push` regardless of remote, R-008 reads JSONL files directly without cloud, R-006 (CI mirror) is explicitly opt-in, R-501 (`ag-mcp serve`) is for users who need cloud orchestration, not the default. (Already credited in Sprint 1 entry; lifted as cross-cutting V5 principle.)

#### Thread 5 — Iterative pushback against single-agent-hooks defaults

**User insight 17 — Repeatedly rejected agent-proposed plans that defaulted back to single-agent hooks**: The redesign blueprint went through five revisions (v1 → v5) over the 2026-04-25 → 2026-04-26 cycle precisely because each agent-produced draft kept reverting to the framework's existing center of gravity — more in-session hooks, more skills, more advisory PostToolUse layers — even after the empirical case for inversion was established. The user pushed back on draft after draft. Concrete evidence: the *sibling doc* (`2026-04-25-swarm-orchestration-and-close-out-hardening.md`, 4 rounds of agent peer review) ultimately proposed *Tier 1 (close-out gate) + Tier 2 (Agent Teams) within the existing single-agent paradigm.* The V5 plan opens by explicitly rejecting that framing: *"This v5 plan accepts the sibling's empirical findings but rejects single-agent self-enforcement as a primary reliability mechanism."* (V5 plan line 11.)

**User insight 18 — "Why is this plan yet again depending on single-agent hooks?"**: User pattern-matched on the recurrence — every draft, after absorbing the empirical failure, would still anchor "primary reliability mechanism" on a single-session close-out gate. The V5 plan's comparison table at line 942 records the resolution: *Sibling primary reliability mechanism = "Single-session close-out gate (Tier 1 sibling)" → V5 primary reliability mechanism = "Tier 0 local external enforcement (pre-commit + pre-push + `ag merge`)" → Why = months of empirical failure proved single-agent self-enforcement asymptotes below required reliability.* The user had to surface this delta multiple times before it stuck.

**User insight 19 — The "asymptote" framing**: User reframed single-agent enforcement not as *broken* (which would invite "fix the hooks") but as *asymptotically bounded below required reliability* — meaning no amount of additional hooks/skills/instruction-file reinforcement crosses the threshold M.A.-style multi-hour autonomous runs need. This forced the architectural conversation to "what mechanism can cross the threshold" (answer: external enforcement + topology) instead of "which hook needs fixing." Adopted verbatim into V5 plan §"What does NOT work / What DOES work."

**User insight 20 — Pushback as a structural contribution, not a review pass**: The five plan revisions are not editorial polish — each was a content-substantive reframing forced by the user catching the agent defaulting back to the comfort zone. v1 was leaked-architecture lens only; v2 added sibling-doc integration + cross-tool walk-back; v3 added abandoned-ambitions archaeology (~70 catalogued items); v4 added production-gap Theme J; v5 finally inverted the center of gravity. Without the pushback, V5 would have been "v1.5 with more hooks." The redesign cycle was a sequence of "yes but you're still proposing in-session enforcement" until the inversion was actually internalized.

**Why these matter**: V5 is not "more enforcement layered on top of the existing framework." It's a *center-of-gravity inversion* — moving from in-session rule-following (proven empirically to fail) to git-layer + topology + voluntary intelligence (proven by 2026 SOTA). The five threads above are how the user arrived at the inversion: (1) the downstream failure ruled out single-agent enforcement empirically, (2) the M.A. audit established what the field actually does, (3) the Claude architecture paper showed Anthropic's own design choice, (4) the transform-vs-greenfield analysis kept the install-base while still getting the inversion, **(5) repeated pushback against agent-default plans that kept reverting to single-agent hooks forced the inversion to actually happen rather than being absorbed nominally and abandoned in implementation.** Each thread is structurally load-bearing — V5 doesn't work without all five. Thread 5 is what made the architecture decision *durable* against the gravity well of "just add another hook."
...
**User insight 9 — Test that intelligence is used in practice, not just on paper**: "I'd also like tests for creating a project on various domains, and checking that domain specific awareness is used in practice because of the intelligence engine. And also that the other features of that engine are used in reality, not just paper." (Apr 2) — The phrase "not just paper" reframed the entire testing approach. Led to 42 integration tests that create real React/Django/Rust/Go/Rails/Java projects, run bootstrap, verify domain-specific output, test pattern enforcement on realistic file writes, and check that quality checklists actually surface in phase queries. Tests that prove the engine works end-to-end, not just that files exist.

**User insight 10 — Spec quality audit as a testing practice**: "btw are the specs/ac good for it" (Apr 2) — Initiated a simultaneous spec audit alongside testing. Found the contract was stale (lifecycle: implementing, should be shipped), FEATURES.md entry had TODO placeholders, and several ACs were vague ("handled gracefully", "enabling the agent to generate"). Led to tightening 2 ACs, adding 5 new error recovery ACs (AC-038–042), and fixing the FEATURES.md entry. Established the pattern: test the specs, not just the code.

**User insight 11 — Framework observability beyond intelligence**: "Would be cool to have other logging also for the framework operations, for validation and auditing purposes." (Apr 2) — Broadened the scope from intelligence-specific logging to general framework operation observability. The intel event log became a template for how all framework subsystems should track what they do — structured events, session finalization, lifetime aggregates.

**User insight 12 — Challenge unnecessary commands**: "Why do we need the ag cap command?" (Apr 3) — Challenged the proposed `ag cap` command with 5 ACs as unnecessary indirection. The agent can just edit FEATURES.md directly. The hooks are the enforcement; the agent is the executor. No new command needed. Removed 5 ACs worth of complexity.

**User insight 13 — Reject dual formats**: "I don't like that there is a dual format." (Apr 3) — Rejected the discovery/formal dual-format approach in FEATURES.md. A capability catalog should have ONE format. Led to the realization that the catalog and the spec system are separate concerns — the catalog says what exists, contracts say what the acceptance criteria are.

**User insight 14 — Framework principles live in repo files, not local memory**: "No, not the memory which is only LOCAL." (Apr 3) — When the agent saved the enforcement hierarchy to local auto-memory, corrected that framework-wide principles must live in repo instruction files (CLAUDE.md template, cursorrules, copilot, codex, memory-seed) so ALL users and agents get them. Local memory is for personal preferences; repo files are for shared principles.

**User insight 15 — Document enforcement hierarchy for future agents**: "Also edit our DOCUMENTATION so that you don't go to the pre-commit hook route in the future." (Apr 3) — Not just "fix this now" but "make sure future agents don't repeat this mistake." The enforcement hierarchy must be documented in instruction files that agents load, not just in FRAMEWORK_DEVELOPMENT.md that only framework devs read.

**User insight 16 — Graduation framing must be honest**: Challenged "will `ag specs` be able to use this during graduation?" as a design test — "does that make any sense?" (Apr 3) — Called out that `ag specs` doesn't exist and the framing implies discovery artifacts only have value as formal spec input. The real test: are artifacts understandable months later? Graduation is a side benefit of good structure, not the purpose.
...
Tomas's key directive: **replace behavioral instructions with structural enforcement**. Instead of writing more instructions telling agents to follow a workflow, build a CLI state machine that makes it impossible to skip steps. The CLI blocks invalid transitions (you can't move to `implementation` without `plan.md`), loads role-specific guidance just-in-time (agents get the right prompt at each phase), and creates an audit trail (every transition logged in `item.yaml`). This is the enforcement hierarchy principle taken to its logical conclusion — if behavioral rules get skipped repeatedly (and they do, because LLMs are probabilistic), the answer isn't "add it to more instruction files," it's "make it a structural gate that can't be bypassed."

The v2 engine design has three layers: (1) `state_machine_af.yaml` — a single YAML file defining 10 states, transitions, modes, and profiles, replacing 35 workflow docs + 10 checklists + 9 quality standards; (2) 7 role prompts (~50 lines each) loaded JIT at transitions, replacing 35 subagent definitions + 13 multi-page skills; (3) per-feature work items (`.agentic/work/F-XXXX/`) with artifact-based preconditions, replacing scattered state across FEATURES.md, STATUS.md, and journal files.
...
### Enforcement Hierarchy as Design Principle (F-0239, PR #173)

**User insight**: Tomas provided the plan that identified a recurring meta-pattern across the framework's history: behavioral rules get skipped repeatedly (20+ doc update skips, 3+ instruction file skips, dogfood skips), while script-enforced rules have zero bypasses. The plan codified this into a 4-tier enforcement hierarchy (automated chaining > blocking gates > state-based detection > behavioral guidance) with a concrete "promotion rule": if a behavioral rule has been skipped 3+ times across sessions, promote it to a higher enforcement level. The key insight: repeated failures are evidence of *misclassification* — the rule was placed at the wrong enforcement tier — not evidence of insufficient documentation. This reframes the response to agent non-compliance from "add it to more instruction files" to "promote it up the hierarchy." The plan also identified the specific root cause of the dogfood skip: the agent used raw `gh pr merge` instead of `ag merge`, bypassing the structural chain that would have run `ag done` automatically. Two hook-based detections now catch this bypass vector structurally.

### Specs Are Contracts AND Living Documents (F-0237, PR #171)

**User insight**: During implementation of the doc decision tree, Tomas caught that the code changes affected shipped specs (F-0138, F-0139, F-0207) but their acceptance criteria hadn't been evolved. When asked "should we update ACs?", Tomas said: "those features' specs evolve as we have learned more how they should work." This reframed shipped spec protection from pure defense to defense + evolution: specs are contracts (never modify without migration) AND living documents (must evolve when understanding deepens). Tomas then directed two additional structural changes: (1) "mark in the framework instruction files, not just memory" — spec evolution must be in all instruction files so agents can't resist it, and (2) "the review should also review this aspect" — the reviewing-code skill now checks whether changes affect shipped specs (dimension 8). Finally, Tomas clarified the scope boundary: "these migrations naturally are done only when we have specs+AC writing (in formal modes)" — all spec evolution steps are conditional on `feature_tracking: yes`. The result: Migration 017 evolved 3 shipped specs, and "Evolve Affected Specs" became an explicit step in implementation (Step 7), commit (backstop), completion (Step 5c), and review workflows across all 11 instruction files.

### Documentation as First-Class Implementation Step (F-0237, PR #171)

**User insight**: Tomas provided the plan for restructuring doc updates from a vague one-liner into a concrete 3-concern decision tree. The key insight: "docs" means two things and agents conflate them — (1) project documentation (CHANGELOG, HOW_IT_WORKS) and (2) instruction files (CLAUDE.md, cursorrules). The plan specified exact file paths for each concern so agents can't skip them. Tomas also identified that `FRAMEWORK_WORKFLOW.md` was missing from the docs registry — an example of the very gap the decision tree was designed to catch. The approach: make instructions concrete enough that they work without enforcement, rather than adding more structural gates. "Maybe the instructions would be enough if expressed well enough."

### Autonomous Formal Profile + Full Pipeline Execution (v0.58.0+)

**User insight**: Tomas requested the framework be configured so that taking a backlog task automatically flows through plan-reviewing, implementing, testing, documentation, PR creation, and PR review — alerting him only when the PR is ready for merge. This validated the `autonomous_formal` profile as the right abstraction: formal rigor (spec-first, dialectical plan review, quality gates) with agent-driven reviews (`review_code: critical_agent`, `review_commit: critical_agent`, `review_regression: critical_agent`) and only `review_merge: human` as the stopping point. Three key decisions Tomas confirmed: (1) profile switch over individual setting overrides (cleaner), (2) AI-reviewed regressions are acceptable even for shipped spec contracts, (3) human stays at merge gate only. The session demonstrated the pipeline end-to-end: F-0216 went through dialectical review (Critic found 5 issues, Advocate validated 9 "already satisfied" claims), implementation, 34 new tests, critical agent PR review (caught gen_args override bug + missing LLM test), auto-fix, and PR delivery. F-0217 and F-0218 were discovered to be already 90%+ implemented and shipped with minimal fixes.

### Pipeline Summary as Structural Workflow Step

**User insight**: After F-0216's PR delivery, Tomas said the phase-by-phase summary ("plan review → implementation → tests → PR → critical agent review → fixes") was valuable. When asked to remember it, he specifically said "not memory" — meaning this should be a structural part of the workflow, not a personal preference stored in auto-memory. This led to adding Step 6.5 (Pipeline Summary) to the `committing-changes` skill (root + template), making it a framework-wide behavior: all agents present a numbered summary of pipeline phases executed when handing off a PR. The distinction between "save as memory" vs "make it structural" reflects a core framework principle: behaviors that should be universal go in instruction files, not personal preferences.

### Dogfood Drift Must Be Investigated, Not Dismissed

**User insight**: Tomas caught the agent dismissing dogfood Phase 1 drift as "advisory, pre-existing" without investigating what it actually was. The drift turned out to be 18 missing command references across 6 instruction files — real gaps where agents wouldn't know about `ag test`, `ag transition`, `ag audit`, etc. The correction: dogfood drift reports should always be investigated (read the actual output), even if Phase 1 is advisory. If the fix is too large for the current PR, log it as a TODO with full context and do it next — don't wave it off.

### Auto-Continue After Plan Mode + Convergence-Aware Flow

**User insight**: Tomas identified two compounding gaps from F-0234 (ExitPlanMode hook) and F-0236 (convergence loop): (1) the hook script existed but was never wired up in `.claude/hooks.json`, so the ExitPlanMode hook never actually fired in Claude Code; (2) even with `plan_review_convergence: auto` set, all 14 instruction files still said "wait for user" / "user decides Proceed/Revise/Reject" — the auto-convergence setting was ignored by every instruction that guides agent behavior. The result: after plan mode, the agent would stop dead and wait for manual acceptance of the DRAFT, then often forget to run the review loop entirely. Tomas's key directive: the agent should never stop between plan creation and review — auto-continue through save → Critic + Advocate → synthesis → convergence check. The only stopping point is in `manual` mode after presenting synthesis, or in `auto` mode only if max iterations reached (→ ESCALATED). Fixed by: (1) creating `.claude/hooks.json` with ExitPlanMode PostToolUse matcher, (2) updating all 15 instruction files to replace "wait for user" with convergence-aware flow, (3) updating hook script output to say "AUTO-CONTINUE" instead of "run ag implement."

### Plan Revisions Must Not Be Deferred to Implementation

**User insight**: Tomas caught the agent recommending "Proceed with the refinements above incorporated during implementation" after a dialectical review that identified 6 substantive refinements (awk sorting → section reordering, unspecified field mappings, missing help text, ambiguous limit application point, etc.). The problem: "Proceed with caveats" is a soft approval that bypasses the rigor the review was designed to enforce. The plan is the contract for implementation — if refinements are deferred, the implementer (possibly a different agent or future session) won't have the full picture. The correct behavior: if synthesis has actionable refinements, the default recommendation is "Revise", and the plan gets perfected before approval. This rule was added across all 8 files that define the dialectical review workflow: CLAUDE.md (root + template), memory-seed, agent_operating_guidelines, dialectical_review.md, plan_review_loop.md, and both planning-features skill references. Added as both a synthesis rule (#6: "Refinements belong in the plan, not in implementation") and a wrong rationalization ("Proceed with refinements during implementation").

### Post-Merge Dogfood Sync + Deferred Item Capture (v0.58.0)

**User insight**: Tomas identified two systematic failure modes during a dogfood audit of recently shipped features (F-0234, F-0224, F-0193): (1) Framework PRs change templates but root instruction files drift — the smoke-test-evidence rule shipped in the template CLAUDE.md but was never backported to root, so framework developers themselves didn't follow it. Quick Commands across all 8 instruction files (root + template × 4 tools) had diverged with missing `ag merge`, `ag verify`, `ag feedback`, `ag formalize`, etc. (2) Future work items mentioned in PR descriptions and plans ("wrappers for other tools", "PR 2 for other ID types", "field validation needed") stayed buried in prose — nobody runs `ag todo` for them, so they're rediscovered only when someone rereads old plans. Tomas's key directive: when you have context, write it with the TODO — a one-liner without Background and Related fields is unactionable guesswork later. The fix was structural: post-merge dogfood sync step added to the completing-work skill and feature_complete checklist (compare root vs templates, update memory-seed, propagate checklist changes to skill references), plus mandatory deferred-item capture with context at both commit and completion time. Both rules are reinforced at 5+ layers (skills, checklists, skill references, memory-seed, feedback memory).

### Plan-Review Reliability via Tool-Native Hooks (F-0234)

**User insight**: Tomas identified that 6+ layers of prompt reinforcement for plan-save-and-review after ExitPlanMode still weren't reliable — agents skip steps despite memory-seed, CLAUDE.md, skills, auto_orchestration, and artifact embedding. His research (`docs/research/2026-03-17-plan-review-reliablity.md`) confirmed what was already framework principle #1: external/structural enforcement has a higher ceiling than prompt-based enforcement. The key design decision: use Claude Code's PostToolUse hook with a tool-name matcher (`"matcher": "ExitPlanMode"`) to inject instructions at the exact transition point, paired with a pre-commit gate (Check 21) as a universal backstop. The shared hook logic in `.agentic/lib/hooks/shared/` was designed agent-agnostic from the start to support future Gemini/Codex/Cursor wrappers. The architecture doc update establishes tool-native hooks as a recognized enforcement category alongside git hooks, with a transition points table mapping future hook opportunities.
...
### Framework System Map — Comprehensive Documentation (docs/framework-system-map branch)

**User insight**: Tomas initiated and directed the creation of `docs/FRAMEWORK_WORKFLOW.md` — a multi-zoom-level reference document covering the entire framework. The core design decision: structure the document at three zoom levels (30-second overview, 5-minute walkthrough, deep reference) so different audiences can enter at the right depth. After the initial 1,136-line document was created, Tomas required a full 4-round dialectical review (Critic + Advocate), which caught 7 factual errors: feature counts were wrong (2 implementing vs. actual 12 including legacy in_progress), shell tool count was inconsistent across tables, git tags were 70 not 92, the auto module table had wrong entries, and ADR cross-links were missing. This wasn't cosmetic — the document is a reference, so wrong numbers propagate as wrong assumptions. Tomas then directed Section 1b (idea-to-spec pipeline): showing the full transformation from raw idea to implementation-ready spec with concrete artifact examples at each stage, multiple entry points (todo/backlog/kickoff/decompose/formalize), and Discovery vs. Formal comparison — because the original document jumped from "idea" to "plan" without showing how the idea materializes into spec artifacts.

Finally, Tomas identified 5 pipeline gaps where the document described *what* exists but not *how things transform*: (1) how feature specs + NFRs combine into ACs, (2) how ACs become unit/integration/e2e/LLM tests, (3) the verification fix-loop mechanics, (4) the research/exploration phase before planning, and (5) project-dependent context generation. The key design directive for the 4 new deep-pipeline sections: thread a single concrete example (F-0042: User Login) through ALL sections so the reader follows one feature from NFR derivation → AC writing → test type decisions → verification loop → commit. This "running example" approach prevents the common documentation failure of describing machinery in isolation without showing how outputs connect to inputs at the next stage.
...
Tomas had to provide three layers of correction: (1) **Workflow expectations were hardcoded per profile name** in scenario YAML — "if formal, check plans" — but profiles are presets, not identities. A user can set `profile: discovery` with `feature_tracking: yes`, and expectations must adapt to the resolved settings, not the label. The fix: `derive_workflow_expectations()` calls `get_setting()` with its 3-level fallback. (2) **The test project was missing the entire agent bootstrap** — the most critical gap. The fix: call the real `setup-agent.sh` and `generate-skills.sh` scripts (the same ones `install.sh` uses) so the test project matches what real users get. (3) **Don't reimplement init logic in Python** — when the agent's first attempt was to write a custom Python function that manually copied CLAUDE.md and iterated over skill directories, Tomas corrected this: use the actual init scripts, not a reimplementation, because (a) the test should exercise the real code path and (b) reimplementations drift from the source of truth. The broader lesson: a verification system is only as real as its setup. If the test environment doesn't match what users actually get, you're testing a fiction.

### Parallel Epic Execution with Worktrees (F-0214)

**User insight**: Tomas identified the sequential epic execution bottleneck — 5 children at ~10min each means ~50min serial vs ~10min parallel — and drove the design of `ag auto epic F-XXXX --parallel` as the solution. The key architectural decision was reusing existing infrastructure (worktree.sh, AGENTS.json with fcntl locking, `build_claude_cmd()`) rather than building new concurrency primitives. During post-implementation code review, Tomas caught three issues: (1) `spawn_claude_async` was declared in `__init__.py` but never called — `ParallelDispatcher._spawn_feature` builds its own Popen inline, making the function dead code with an unclosed file handle; (2) `_close_log` tried to close `agent.process.stdout`, which is `None` when stdout is redirected to a file object — the actual file handle from `open(log_path)` was lost; (3) `completed_ids` was populated but never read. The fix pattern: track the file handle on the `AgentProcess` dataclass itself (`log_file` field), close it explicitly in `_close_log`, and remove the dead code. Tomas also drove the strengthening of LLM test requirements from "advisory" to "mandatory" for behavioral changes — recognizing that the LLM layer decides whether deterministic code ever gets called, so without an LLM test there's no proof agents will use a feature.
...
**User insight**: Tomas proposed a self-healing framework verification system: the agent builds real example projects (todo app, API service, CLI tool) end-to-end using the framework, acting as developer/architect. When a framework bug surfaces during the build, the agent (1) fixes the framework code, (2) restarts the example project from scratch, (3) repeats until the full lifecycle completes without errors. This creates a closed loop — build → hit bug → fix → rebuild → verify — that catches behavioral gaps unit tests and LLM tests structurally cannot. Key design constraints from Tomas: hard guards against touching main/real branches (worktrees and ephemeral branches only), automatic cleanup, and abort on any main-branch mutation attempt. **Prerequisite identified by Tomas**: scripts like `state-commit.sh` that hardcode `main`/`master` as the trunk branch must accept a configurable trunk so verification runs against ephemeral branches don't pollute real main. **Coverage requirement from Tomas**: the verification loop must exercise all three operation modes (tech lead/formal, visioneer/interview, full autonomous) and the full settings matrix — discovery vs formal profiles, all review modes (human/critical_agent/skip), docs_mode (inline/deferred), worktree_mode, kickoff_confirm (ask/skip), git_workflow (direct/pull_request). This ensures the framework works across all configurations, not just the framework-dev default. **Delivery model from Tomas**: the verification loop runs on an ephemeral trunk branch where agents auto-merge their fix PRs during the loop; once the full cycle passes clean, the accumulated fixes are delivered as a single PR against real main for human review — keeping the self-healing loop autonomous while preserving human approval on the final output.
...
### Autogenerated Project-Specific Subagent Roles (T-0092, v0.71.1)

**User insight**: After seeing how generic Critic/Advocate review loops work, Tomas asked whether production projects could have project-derived role personas participating in plan review and implementation feedback — e.g. a game project automatically scaffolding a game designer, graphics programmer, and engine programmer; a web app getting a UX expert, accessibility lead, and backend architect. The key design question is whether roles are inferred from project type (STACK.md) or explicitly defined, and how they integrate with the existing dialectical review loop. This extends the principle of "agents have specific context" (context-for-role.sh) to the design phase itself.

---

### No-Feature-Inflation Rule + Behavioral Corrections to Instruction Files (v0.71.0)

**User insight**: Tomas explicitly stopped agents multiple times when they proposed new F-XXXX IDs for work that was actually improvement or enforcement of an existing feature. The resulting rule — "improvements, enforcement, and hardening of existing features are deliverables on those features, not new F-XXXX" — was encoded into all instruction files and memory-seed. The framing "which existing feature owns this?" before proposing a new capability ID came directly from this guidance.

**User insight**: Tomas caught agents writing behavioral corrections as memory entries instead of into instruction files, and directed that the rule itself be added to CLAUDE.md, cursorrules, codex, copilot, and the framework's CLAUDE.md template. This is the "corrections go in instruction files, not memory" meta-rule — which Tomas also insisted be added to the framework template so it propagates to user projects.

---

### NHL Hockey Game as Framework Probe + Multi-Agent Collision Evidence (v0.70.0)

**User insight**: Tomas ran an NHL hockey game project (two-player browser game, autonomous_formal) as a second systematic framework probe, immediately after the Street Fury/GTA probe. The methodology refinement here: Tomas pre-copied the project output into the framework repo (`agentic-tests/nhl-hockey-game`) including the JSONL session logs, so the analysis could be done against actual tool-call evidence rather than reconstructed behavior. He explicitly asked for JSONL-backed analysis because "agents may cheat by looking at instructions for the correct answer instead of measuring the BEHAVIOR."

**User insight**: During the analysis, Tomas caught in real time that agents were destroying each other's uncommitted work — an agent wiped local changes that another active agent had made. He stopped the session with: "Did you just remove the other agent's work? At least you should understand if there are file changes that you didn't make, those cannot be wiped out!" This live collision provided the concrete evidence that drove the multi-session safety rules (`count-others` check, AGENTS.json, worktree-by-default).
...
### Real Behavioral LLM Tests Over Unit-Test-Style Checks (F-0242, v0.69.0)

**User insight**: When reviewing the initial LLM test suite for workflow verification, Tomas pushed back on the approach: "The tests should naturally test in real workflow, and not (only) with unit test approach where the agent may cheat by looking at instructions for the 'correct' answer instead of measuring the BEHAVIOR (which includes easily skipping instructions or context compression) in a real world situation." He specifically identified the failure mode: an agent can pass a unit test for "does plan review happen?" by consulting the instruction that says it should, without actually running it. Real behavioral tests must observe tool calls and session artifacts, not ask the agent to self-report.

---

### Backlog ID Confusion → Feature Number Scope (v0.69.0)

**User insight**: Tomas identified that using feature IDs (F-XXXX) as backlog item identifiers was causing agents to treat every backlog entry as a distinct feature capability to implement — creating spurious F-XXXX entries for what were actually sub-tasks or additions to existing features. He directed a cleanup of the backlog and asked: "Will that confuse the agents to create new features of each even though they might be additions to features? If changing, take care not to lose any references/plans related to them." This fed directly into the no-feature-inflation rule.
...
### Cursor Upgrade Lifecycle + Sentinel State Hygiene (v0.79.x → v0.80.x)

**User insight 1 — "Can you check that they would be installed / synced with the init/upgrade/sync processes?"**: After the Cursor upgrade was installed, Tomas immediately asked whether the new `.cursor/` artifacts were wired into upgrade, sync, and init — not just generated once. They weren't: `upgrade.sh` had a step for Claude but not Cursor, and `sync.sh`/`check-environment.sh` only checked `.cursorrules` staleness. **The principle: generating an artifact once is not enough — it needs a regeneration owner or it will drift on every upgrade.**

**User insight 2 — "Should the current changed files in /session /intel be .gitignored?"**: Tomas noticed session event logs, sentinel flags, and `intel/token-summary.json` appearing as untracked. The distinction: these are runtime artifacts with single-session lifetime, not persistent project state. The question forced a clear classification of what lives in `.agentic/session/` and `.agentic/intel/` — derived/transient vs. durable/committed.

**User insight 3 — "Those sentinel files probably should be cleared after session?"**: Tomas identified that sentinel flags (`.plan-review-skipped`, `.correction_hint_shown`, etc.) needed explicit session-end cleanup, not just gitignoring. This led to discovering the Cursor Stop hook was missing `.correction_hint_shown` — a parity gap vs. Claude's `Stop.sh`.

**User insight 4 — "Or at session start too"**: Tomas extended the cleanup to session start as well — recognising that Stop hook cleanup alone isn't sufficient if the session crashes or is force-quit. **The two-mechanism invariant: session-scoped state needs a normal-path cleaner (Stop) AND a crash-recovery cleaner (SessionStart). One mechanism is a single point of failure.**

**Why it matters**: All four insights share the same underlying pattern — state that is created needs an explicit owner for its full lifecycle: generation, regeneration on upgrade, drift detection, and cleanup. The Cursor upgrade exposed that the framework had the generation side right but not the maintenance side.

**Design direction**: Every generated artifact needs a regeneration owner in the upgrade path. Session-scoped sentinels must be cleared by two mechanisms. Runtime artifacts belong in `.gitignore`, not tracked as project state.
