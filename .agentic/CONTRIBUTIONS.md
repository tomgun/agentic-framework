...
**User insight**: Tomas distilled the 8 strategic architectural patterns that make this framework work — not from theory, but from 53 framework versions and ~1400 commits of hands-on building, testing, breaking, and rebuilding. Each insight was earned through real failures: plans lost because they weren't git-tracked, rules ignored because instruction files were too long, agents skipping workflows because triggers were keyword-only without intent matching, state files corrupted because agents did read-modify-write instead of using scripts. The patterns — tiny instruction files + memory reinforcement, skills with frontmatter, scripts over instructions, structural gates via hooks, keyword + intent trigger tables, durable git-tracked state, distributed enforcement, and LLM behavioral testing — form a coherent system where each piece compensates for the others' weaknesses. These aren't framework-specific tricks; they're transferable principles for anyone building agentic coding workflows. To the architect of this framework: these insights exist because you insisted on learning why things failed, not just fixing them.
