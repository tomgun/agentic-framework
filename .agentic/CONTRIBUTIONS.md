...
**User insight**: Tomas drove the full 3-round dialectical review process for the coordination server plan, catching critical design flaws that would have shipped broken. Round 1 caught the biggest naming issue: the original plan called this "MCP Server" but it's not the Model Context Protocol — it's a plain JSON-RPC HTTP API, and calling it MCP would mislead users expecting Claude Desktop integration. Tomas also rejected SSE subscriptions (fragile on stdlib) in favor of stateless `poll_changes` with file mtime, and mandated auth-by-default (bearer token). Round 2 caught that `fcntl.flock` doesn't protect threads within the same process — two threads can both acquire the same flock — requiring an additional `threading.Lock` for intra-process serialization. Round 3 caught the most subtle issue: `poll_changes` with per-client server-side snapshots would have unbounded memory growth because the bearer token is shared (no client identity) — redesigned as fully stateless with `since` timestamp as sole cursor. Tomas also insisted on intelligent testing after the initial shallow test suite ("this must be documented as well and have intelligent testing"), and post-PR code review caught 4 more issues: `transition_state` wasn't actually enforcing `review_mode=skip` (the `enforce=False` param only disabled state enforcement, not review checkpoints), the SIGTERM handler used `sys.exit(0)` in a lambda (can deadlock if a lock is held), double cleanup from atexit + finally, and non-constant-time token comparison.
