...
**User insight**: Tomas provided the complete QA Observatory plan addressing three concerns he'd identified: (1) no central map of what's tested across the framework's 12+ QA categories, (2) no way to verify intermediate workflow states (only final outcomes), and (3) no empirical evidence that the framework's complexity actually helps — simpler configurations might work just as well. The plan's most provocative element was Part 3 (Complexity Tier Experiments): run the same task across discovery/formal/autonomous_formal profiles and compare universal metrics (app works, tests pass, code quality) to answer "does the complexity justify itself?" The key design decisions Tomas made: (a) generated registry over hand-written (156 features × 12 categories would drift instantly), (b) framework.log as a tool-agnostic complement to Claude's JSONL (respecting agent-agnostic principle D7), (c) universal vs framework-specific metric separation so tier comparisons don't circularly measure framework features as a proxy for framework value, (d) three real configuration profiles as tiers (not a synthetic "CLAUDE.md only" strawman — that was revised during dialectical review). The plan was structured as an epic with honest dependency ordering: F-0241 independent, F-0240→F-0242→F-0243 sequential — explicitly rejecting the initial claim of parallel execution.
