...
**User insight**: Tomas framed the three core concerns that drove this initiative: (1) no central map of what's tested across the framework's 12+ QA categories, (2) no way to verify intermediate workflow states (only final outcomes), and (3) no empirical evidence that the framework's complexity actually helps — simpler configurations might work just as well. The third concern is the most provocative: the framework might be over-engineered, and Tomas wanted empirical data to answer that question rather than assuming complexity is justified. During implementation, Tomas enforced process discipline: when the agent tried to skip plan review and jump straight to coding, Tomas corrected with "save the plan first, review it" — ensuring the framework's own dialectical review process was followed. When the agent then stopped to ask for human approval before committing (despite `review_commit: critical_agent` being set), Tomas again corrected: "what do the settings say? why are you not doing the framework" — pointing out that the autonomous_formal profile means the agent should commit directly with adversarial review, not wait for manual approval. Both corrections reinforced that the framework's own rules apply to framework development.
