...
### Settings-Driven Workflow Expectations & Agent Bootstrap (F-0215)

**User insight**: Tomas identified two fundamental gaps in the framework verification system. First, workflow expectations were hardcoded per profile name in scenario YAML files (e.g., "if formal, check plans"), but profiles are just presets — a user can set `profile: discovery` and override `feature_tracking: yes`, and the expectations should adapt. The fix: derive expectations at runtime from `get_setting()` with its 3-level fallback (explicit → preset → default), so a discovery project with `feature_tracking: yes` gets feature checks while a formal project with `plan_review_enabled: no` skips plan checks. This decouples verification from profile identity and ties it to the actual resolved configuration. Second — and more critically — Tomas pointed out that the test project setup was missing the agent bootstrap entirely: `setup_project()` copied `.agentic/lib/` (the framework internals) but never created CLAUDE.md, `.claude/skills/`, or AGENTS.md — the files Claude Code auto-reads. The spawned build agent was operating without the framework's behavioral contract, relying solely on the minimal `verify_build.md` prompt. The fix: `_bootstrap_agent_files()` replicates what `setup-agent.sh claude` + `generate-skills.sh` do at install time — copying the CLAUDE.md template, generating skills from source, and creating AGENTS.md — so the build agent has the same instruction surface as a real user project.
