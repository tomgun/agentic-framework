...
#
# Usage:
#   bash .agentic/lib/hooks/session-start.sh
#
# Output:
...
if [[ -x ".agentic/lib/tools/wip.sh" ]]; then
  if bash .agentic/lib/tools/wip.sh check; then
...
if [[ -f "STATUS.md" ]]; then
  ...
  if [[ $STATUS_AGE -gt 7 ]]; then
    ...
    echo ""
    if [[ -x ".agentic/lib/tools/status.sh" ]]; then
      bash .agentic/lib/tools/status.sh infer 2>/dev/null || true
      echo ""
      echo "   → Review the inferred state above."
      echo "   → To apply: bash .agentic/lib/tools/status.sh infer --apply"
...
if [[ "$FT" == "yes" ]]; then
  ...
  echo "- .agentic/spec/acceptance/F-####.md - If working on specific feature"
...
if [[ -f "STATUS.md" ]]; then
  ...
  echo "⚠️  No STATUS.md found - run: bash .agentic/lib/init/scaffold.sh"
fi
...
if [[ "$FT" == "yes" ]] && [[ -f ".agentic/spec/FEATURES.md" ]]; then
  SHIPPED_NOT_ACCEPTED=$(grep -A5 "^## F-" .agentic/spec/FEATURES.md | grep -B5 "Status: shipped" | grep -B5 "Accepted: no" | grep "^## F-" || echo "")
