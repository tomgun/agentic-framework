...
if [[ -f "$FEATURES_FILE" && -d "$AC_DIR" ]]; then
  # Extract feature IDs from heading lines only (## F-XXXX:) to avoid
  # spurious matches from dependency/reference mentions in body text.
  # -B 3: heading may be 2-3 lines before **Status** (blank line, category, etc.)
