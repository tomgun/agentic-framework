...
- `ag hooks register` (and `ag hooks install`) also installs a Claude Code statusline showing `project | branch | ctx N% | N% 5h, M% 7d - reset HH:MM, Day HH:MM TZ (updated at main agent response) | task`. The project label is derived from `git remote.origin.url` so it survives Docker mount-point dirs like `/workspace`. The rate-limit cluster is zero-config — read straight from Claude Code's `rate_limits` envelope (Anthropic API rate-limit headers, snapshot from the last main-agent turn). One shared timezone label is appended to the reset block. The dim `(updated at main agent response)` trailer anchors that the values aren't live; `/usage` triggers a refresh. Pass `--force-statusline` to overwrite an existing custom statusLine.
