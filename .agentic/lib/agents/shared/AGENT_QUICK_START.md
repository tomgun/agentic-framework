...
| Phase | When | Command |
|-------|------|---------|
| START | Session begins | `bash .agentic/lib/tools/doctor.sh` |
| PLANNING | User says "implement F-####" | Check `.agentic/spec/acceptance/F-####.md` exists |
| BUG FIX | User reports or says "fix", "bug", "issue" | Log to `.agentic/spec/ISSUES.md` first, then fix |
| IMPLEMENT | Acceptance exists | Write code + tests |
| COMPLETE | User says "done" | `bash .agentic/lib/tools/doctor.sh --full` |
| COMMIT | Ready to commit | `bash .agentic/lib/tools/doctor.sh --pre-commit` |
...
Add to `.agentic/HUMAN_NEEDED.md`. Ask the human.
...
`.agentic/lib/agents/roles/orchestrator-agent.md`
...
- Full guidelines: `.agentic/lib/agents/shared/agent_operating_guidelines.md`
- Principles: `.agentic/lib/PRINCIPLES.md`
- Checklists: `.agentic/lib/checklists/`
