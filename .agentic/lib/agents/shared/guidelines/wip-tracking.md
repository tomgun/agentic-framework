...
bash .agentic/lib/tools/wip.sh start F-0005 "User authentication" "src/auth/*.ts,tests/auth/*.test.ts"

# Starting work (Core profile)
bash .agentic/lib/tools/wip.sh start "auth-implementation" "User login and JWT tokens" "src/auth/*.ts"
```

### Update WIP frequently (~every 15 minutes)

```bash
bash .agentic/lib/tools/wip.sh checkpoint "Login endpoint complete, starting JWT validation"
bash .agentic/lib/tools/wip.sh checkpoint "Unit tests passing, working on integration tests"
```
...
### Before switching:
```bash
bash .agentic/lib/tools/wip.sh checkpoint "Switching from Claude to Cursor"
```
