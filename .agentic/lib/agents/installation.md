...
|------|------------------|---------------|
| Claude Code | `CLAUDE.md` | `bash .agentic/lib/tools/setup-agent.sh claude` |
| Cursor | `.cursorrules` | `bash .agentic/lib/tools/setup-agent.sh cursor` |
| GitHub Copilot | `.github/copilot-instructions.md` | `bash .agentic/lib/tools/setup-agent.sh copilot` |
| **All tools** | All of the above | `bash .agentic/lib/tools/setup-agent.sh all` |
...
bash .agentic/lib/tools/setup-agent.sh claude   # Claude Code
bash .agentic/lib/tools/setup-agent.sh cursor   # Cursor
bash .agentic/lib/tools/setup-agent.sh copilot  # GitHub Copilot
...
- Points to `.agentic/lib/agents/shared/agent_operating_guidelines.md`
...
```bash
mkdir -p .codex
cp .agentic/lib/agents/codex/codex-instructions.md .codex/instructions.md
```
...
```bash
mkdir -p .gemini
cp .agentic/lib/agents/shared/agent_operating_guidelines.md .gemini/instructions.md
```
...
2. Copy the content from `.agentic/lib/agents/shared/agent_operating_guidelines.md`
...
If you update `.agentic/lib/agents/*/` content, re-run setup:
