...
3. Fill in `STACK.md`, `CONTEXT_PACK.MD`, `.agentic/STATUS.md`, and specs
4. Set up quality checks (if applicable)
5. Configure Git workflow
6. Get you ready to develop

**You don't need to run any scripts manually.** The agent follows `.agentic/lib/init/init_playbook.md` automatically.
...
- `.agentic/STATUS.md` - Current state and roadmap
- `CONTEXT_PACK.md` - Architecture snapshot
- `JOURNAL.md` - Progress log
- `.agentic/HUMAN_NEEDED.md` - Items needing human input
...
1. `.agentic/STATUS.md` (current focus, next steps)
2. `CONTEXT_PACK.md` (where things are, how to run)
3. `JOURNAL.md` last 2-3 entries (recent progress)
4. `.agentic/HUMAN_NEEDED.md` (blockers requiring attention)
...
- [`.agentic/STATUS.md`](../../../STATUS.md): Current focus, next steps, roadmap
...
- [`.agentic/spec/FEATURES.md`](../../../.agentic/spec/FEATURES.md): Feature registry with acceptance
- [`.agentic/spec/NFR.md`](../../../.agentic/spec/NFR.md): Non-functional requirements (performance, security)
...
> 
> To regenerate tool files: `bash .agentic/lib/tools/setup-agent.sh all`
...
|--------------|---------------|-----|
| Task / idea / reminder | `.agentic/TODO.md` | `ag todo "description"` |
| Needs human action (PR review, credentials, decision) | `.agentic/HUMAN_NEEDED.md` | `bash .agentic/lib/tools/blocker.sh add` |
| Bug or technical debt | `ISSUES.md` | `bash .agentic/lib/tools/quick_issue.sh` |
| New capability to spec | `FEATURES.md` | `bash .agentic/lib/tools/feature.sh add` |
...
- Current state: `.agentic/STATUS.md`
...
- Existing ADRs: `.agentic/spec/adr/`
...
- `bash .agentic/lib/tools/report.sh` - Feature status
- `bash .agentic/lib/tools/verify.sh` - Comprehensive checks
- `bash .agentic/lib/tools/coverage.sh` - Code annotation coverage
- `bash .agentic/lib/tools/ag.sh trace` - Spec-code traceability (drift + coverage)
- `bash .agentic/lib/tools/retro_check.sh` - Check if retrospective is due
- `python3 .agentic/lib/tools/validate_specs.py` - Validate spec frontmatter
...
| Lost track of what we're building | Read `.agentic/STATUS.md` and `spec/OVERVIEW.md` |
