...
bash .agentic/lib/tools/wip.sh check        # Check interrupted work
bash .agentic/lib/tools/journal.sh ...      # Log to JOURNAL.md
bash .agentic/lib/tools/nfr.sh list         # List NFRs with status
...
- **a) Discovery (Simple Setup)**: Quality standards, multi-agent, research, lightweight planning (`.agentic/OVERVIEW.md`)
...
# Evening
bash .agentic/lib/tools/doctor.sh --full  # Verify
...
**Create `.agentic/spec/acceptance/F-0010.md`:**
```markdown
...
```bash
# Create migration
bash .agentic/lib/tools/migration.sh create "Add CSV Export feature"

# Edit the generated migration file
# .agentic/spec/migrations/042_add_csv_export_feature.md
```
...
**Edit `.agentic/spec/acceptance/F-####.md`** directly with new criteria.
...
```bash
bash .agentic/lib/tools/report.sh
```
...
```bash
bash .agentic/lib/tools/coverage.sh              # Human-readable report
bash .agentic/lib/tools/coverage.sh --json       # Machine-readable output
bash .agentic/lib/tools/coverage.sh --reverse src/auth.py  # Features in this file
bash .agentic/lib/tools/coverage.sh --test-mapping  # Infer test→feature mapping
```
...
```bash
bash .agentic/lib/tools/spec-analyze.sh F-0148    # Analyze one feature
bash .agentic/lib/tools/spec-analyze.sh --help     # Usage info
```
...
```bash
python3 .agentic/lib/tools/coverage.py --ac-coverage F-0148        # Human-readable
python3 .agentic/lib/tools/coverage.py --ac-coverage F-0148 --json # JSON output
```
...
bash .agentic/lib/tools/ag.sh trace --json       # Combined JSON output
```
...
```bash
bash .agentic/lib/tools/feature_graph.sh
# Or save to file:
bash .agentic/lib/tools/feature_graph.sh --save
```
...
```bash
bash .agentic/lib/tools/deps.sh
```

#### `stale.sh` - Staleness Detector (DEPRECATED)

> **Deprecated**: Use `bash .agentic/lib/tools/sync.sh` instead. Staleness checks are now in sync.sh Phase 2.

```bash
bash .agentic/lib/tools/sync.sh --check  # Includes staleness + all other checks
```
...
python .agentic/lib/tools/query_features.py --status=in_progress

# Find auth-related features
python .agentic/lib/tools/query_features.py --tags=auth

# Find critical UI features currently in progress
python .agentic/lib/tools/query_features.py --tags=ui --priority=critical --status=in_progress

# Show counts by category
python .agentic/lib/tools/query_features.py --count

# Filter by owner
python .agentic/lib/tools/query_features.py --owner=alice@example.com
...
python .agentic/lib/tools/feature_graph.py

# Only in-progress features
python .agentic/lib/tools/feature_graph.py --status=in_progress --save

# Only presentation layer
python .agentic/lib/tools/feature_graph.py --layer=presentation

# Features with specific tags
python .agentic/lib/tools/feature_graph.py --tags=auth --tags=ui

# Focus on one feature and its immediate neighbors
python .agentic/lib/tools/feature_graph.py --focus=F-0042 --depth=1

# Show parent-child hierarchy only (no dependencies)
python .agentic/lib/tools/feature_graph.py --hierarchy-only

# Combine filters
python .agentic/lib/tools/feature_graph.py --layer=business-logic --status=planned --save
```
...
```bash
# Validate all specs
python .agentic/lib/tools/validate_specs.py
...
Validating .agentic/spec/FEATURES.md...
...
bash .agentic/lib/tools/accept.sh F-0005

# Mark as accepted without running tests
bash .agentic/lib/tools/accept.sh F-0005 --skip-tests
```
...
```bash
bash .agentic/lib/tools/mutation_test.sh
# Or specific path:
bash .agentic/lib/tools/mutation_test.sh src/auth
```
...
```bash
bash .agentic/lib/tools/retro_check.sh
```
...
```bash
bash .agentic/lib/tools/start.sh
```
...
```bash
bash .agentic/lib/tools/check-environment.sh
```
...
```bash
bash .agentic/lib/tools/framework_age.sh
```
...
```bash
python3 .agentic/lib/tools/accept.py F-0005
```
...
```bash
python3 .agentic/lib/tools/feature_stats.py
python3 .agentic/lib/tools/feature_stats.py --period=30  # Last 30 days
```
...
```bash
python3 .agentic/lib/tools/organize_features.py --by domain --dry-run
python3 .agentic/lib/tools/organize_features.py --by layer
```
...
```bash
python3 .agentic/lib/tools/upgrade_spec_format.py --dry-run
python3 .agentic/lib/tools/upgrade_spec_format.py
```
...
```bash
bash .agentic/lib/tools/context-for-role.sh planning F-0005
bash .agentic/lib/tools/context-for-role.sh implementation F-0005 --dry-run
```
...
**What it does:**
- Copies hand-crafted Claude Skills from `.agentic/lib/agents/claude/skills/` to `.claude/skills/`
...
```bash
bash .agentic/lib/tools/generate-skills.sh           # Generate all skills
bash .agentic/lib/tools/generate-skills.sh --validate # Validate sources only
bash .agentic/lib/tools/generate-skills.sh --clean    # Remove + regenerate
```

**When to run:**
- After modifying skill sources in `.agentic/lib/agents/claude/skills/`
...
```bash
bash .agentic/lib/tools/list-tools.sh
```
...
```bash
bash .agentic/lib/tools/validation-cache.sh check
bash .agentic/lib/tools/validation-cache.sh clear
```
...
```bash
bash .agentic/lib/tools/brief.sh
```
...
```bash
bash .agentic/lib/tools/dashboard.sh
```
...
```bash
bash .agentic/lib/tools/task.sh "Implement retry logic for API calls"
```

Creates `.agentic/spec/tasks/T-####-<slug>.md` from template.

#### `sync_docs.sh` - Generate Doc Scaffolding

```bash
bash .agentic/lib/tools/sync_docs.sh
```
...
```bash
bash .agentic/lib/tools/pipeline_status.sh F-0005
```
...
```bash
bash .agentic/lib/tools/version_check.sh
```
...
bash /tmp/agentic-framework-<VERSION>/.agentic/lib/tools/upgrade.sh /path/to/your-project
```
...
```bash
python3 .agentic/lib/tools/validate_specs.py
```
...
```bash
bash .agentic/lib/tools/whatchanged.sh --days 7
```
...
2. **Profile preset** — from `.agentic/lib/presets/profiles.conf` (set by your profile)
...
- Update `.agentic/OVERVIEW.md` with decisions and completed capabilities.
- Add to `.agentic/HUMAN_NEEDED.md` when blocked.
- Keep `JOURNAL.md` current (session summaries).
- If Formal: keep `.agentic/STATUS.md` and `/spec/*` truthful.
...
Full rules: `.agentic/lib/agents/shared/agent_operating_guidelines.md`
```
...
bash .agentic/lib/tools/doctor.sh --full || exit 1
npm run build
...
grep "Status: planned" .agentic/spec/FEATURES.md
...
bash .agentic/lib/tools/sync.sh --check
...
A: Edit `.agentic/STATUS.md` "Current focus" or tell your agent: *"Make F-#### the priority"*
...
A: Edit `.agentic/spec/acceptance/F-####.md` anytime. Tell agent: *"I updated F-#### acceptance, please adjust implementation"*

**Q: How do I know what's implemented?**
A: Run `bash .agentic/lib/tools/report.sh` or check `.agentic/spec/FEATURES.md` "Implementation: State" field.

**Q: What if agent gets stuck?**
A: Agent should add entry to `.agentic/HUMAN_NEEDED.md`. Check there for blockers.
...
- Update `.agentic/STATUS.md` when focus changes
...
# Weekly
bash .agentic/lib/tools/doctor.sh --full > weekly-health-check.txt
```
...
Implements acceptance criteria from .agentic/spec/acceptance/F-0005.md
...
# Or create manually:
cp .agentic/spec/ADR.template.md .agentic/spec/adr/ADR-0005-use-postgresql.md
```
...
**Add to `.agentic/HUMAN_NEEDED.md` immediately:**
```markdown
...
|--------------|---------------|-----|
| Development idea, task, or reminder | `.agentic/TODO.md` | `ag todo "description"` |
...
- [ ] All tests pass
- [ ] `bash .agentic/lib/tools/doctor.sh --full` passes
- [ ] FEATURES.md updated (implementation state, code paths, test status)
- [ ] JOURNAL.md updated
- [ ] Acceptance criteria met (check .agentic/spec/acceptance/F-####.md)
...
[Planning Agent creates .agentic/spec/acceptance/F-0015.md]
...
```bash
bash .agentic/lib/tools/mutation_test.sh src/payment
```
...
| Before commit | `ag verify` or `bash .agentic/lib/tools/doctor.sh --full` |
| Check feature status | `bash .agentic/lib/tools/report.sh` |
| Find blockers | `cat HUMAN_NEEDED.md` |
| Check test coverage | `bash .agentic/lib/tools/coverage.sh` |
| Visualize dependencies | `bash .agentic/lib/tools/feature_graph.sh` |
| Check health | `bash .agentic/lib/tools/doctor.sh` |
| Check staleness | `bash .agentic/lib/tools/sync.sh --check` |
| Mark feature accepted | `bash .agentic/lib/tools/accept.sh F-####` |
| Check retro due | `bash .agentic/lib/tools/retro_check.sh` |

### Key Files to Bookmark

- `.agentic/STATUS.md` - Current focus, next up, roadmap
- `JOURNAL.md` - Session history (tail -50)
- `.agentic/HUMAN_NEEDED.md` - Blockers and decisions
- `STACK.md` - How to build/test/run
- `CONTEXT_PACK.md` - Architecture overview
- `.agentic/spec/FEATURES.md` - Feature list and status (Formal)
- `.agentic/OVERVIEW.md` - Vision and capabilities (Discovery)
