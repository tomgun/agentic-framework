...
if [[ -f "OVERVIEW.md" ]]; then
  ...
  if [[ "$PRODUCT_LINE_COUNT" -gt 10 ]]; then
    ...
    echo "  - Seed .agentic/spec/FEATURES.md from OVERVIEW.md capabilities"
...
if [[ "$PRODUCT_EXISTS" == "yes" ]]; then
  ...
  echo "      - Seed .agentic/spec/FEATURES.md from OVERVIEW.md capabilities (with F-#### IDs)"
  echo "      - Seed spec/PRD.md from OVERVIEW.md vision and scope\""
else
  echo "  4. Tell your agent:"
  echo "     \"I've enabled the Formal profile. Please review"
  echo "      .agentic/spec/FEATURES.md and help me document our existing features.\""
fi
echo ""
echo "New files:"
echo "  - spec/PRD.md          (Product requirements)"
echo "  - spec/TECH_SPEC.md    (Technical specification)"
echo "  - .agentic/spec/FEATURES.md     (Feature tracking with IDs)"
