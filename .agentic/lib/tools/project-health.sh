...
if [[ -f ".agentic/spec/FEATURES.md" ]]; then
  SHIPPED=$(grep -c "Status: shipped" .agentic/spec/FEATURES.md 2>/dev/null || echo "0")
  IN_PROGRESS=$(grep -c "Status: in_progress" .agentic/spec/FEATURES.md 2>/dev/null || echo "0")
  PLANNED=$(grep -c "Status: planned" .agentic/spec/FEATURES.md 2>/dev/null || echo "0")
  ...
  echo "  No .agentic/spec/FEATURES.md found"
fi
...
if [[ -d "tests" ]] || [[ -d "test" ]] || [[ -d "__tests__" ]]; then
  ...
  if [[ -f ".agentic/spec/FEATURES.md" ]]; then
    SHIPPED_FEATURES=$(grep -B1 "Status: shipped" .agentic/spec/FEATURES.md | grep "## F-" | sed 's/## //' | cut -d: -f1 || true)
    MISSING_TESTS=0
