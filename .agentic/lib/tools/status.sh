...
# STATUS_FILE provided by paths.sh
...
infer_status() {
    ...
    if [[ -f "${PROJECT_ROOT}/.agentic/spec/FEATURES.md" ]]; then
        local in_progress
        in_progress=$(grep -B1 "Status: in_progress" "${PROJECT_ROOT}/.agentic/spec/FEATURES.md" | grep "^## F-" | sed 's/^## //' || echo "")
        ...
        echo "   (no .agentic/spec/FEATURES.md)"
    fi
