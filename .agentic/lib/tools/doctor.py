...
# Legacy constant kept for reference; prefer get_paths(root).verification_state
VERIFICATION_STATE_FILE = ".agentic/session/.verification-state"
...
def write_verification_state(root: Path, result: str, issues: int, suggestions: int, phase: str = None, detected_stack: str = None):
    ...
    state_path = get_paths(root).verification_state
...
def parse_stack_info(root: Path) -> dict:
    ...
    stack_path = get_paths(root).stack_file
...
def checks_for_profile(profile: str, root: Path | None = None) -> list[Check]:
    ...
    journal_relative = str(p.journal_file.relative_to(repo_root))
...
def validate_optional_enhancements(root: Path, profile: str) -> list[str]:
    ...
    status_path = get_paths(root).status_file
...
def run_phase_checks(root: Path, profile: str, phase: str, feature_id: str = None) -> list[str]:
    ...
    if phase == "start":
        ...
        blockers_file = p.human_needed_file
...
def run_summary_check(root: Path) -> dict:
    ...
    agents_file = p.agents_active_file
...
def main() -> int:
    ...
    proposal_files = []
    for check_path in [p.stack_file, p.context_pack_file, p.overview_file]:
        if check_path.exists():
            try:
                if "<!-- PROPOSAL" in check_path.read_text(encoding="utf-8")[:200]:
                    proposal_files.append(str(check_path.relative_to(root)))
            except Exception:
                pass
    if p.features_file.exists():
        try:
            if "<!-- PROPOSAL" in p.features_file.read_text(encoding="utf-8")[:200]:
                proposal_files.append(str(p.features_file.relative_to(root)))
        except Exception:
            pass
