...
check_feature() {
    ...
    if ! grep -q "^## ${FID}:" .agentic/spec/FEATURES.md 2>/dev/null; then
        echo -e "  ${RED}✗${NC} Not found in .agentic/spec/FEATURES.md"
    ...
    local HAS_MIGRATION
    HAS_MIGRATION=$(grep -rl "$FID" .agentic/spec/migrations/*.md 2>/dev/null || true)
...
if [[ "${1:-}" == "--all" ]]; then
    ...
    if [[ ! -f ".agentic/spec/FEATURES.md" ]]; then
        echo -e "${RED}No .agentic/spec/FEATURES.md found${NC}"
        exit 1
    fi

    FEATURES=$(grep -E "^## F-[0-9]+:" .agentic/spec/FEATURES.md | sed 's/^## //' | cut -d: -f1)
    TOTAL=0
