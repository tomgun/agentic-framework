...
- Requirements: [R-####, ... | none]
- NFRs: [NFR-####, ... | none]
- Acceptance: [.agentic/spec/acceptance/F-####.md | todo | tbd]
- Verification:
  - Accepted: [yes | no]
...
| **Acceptance** | reference | `.agentic/spec/acceptance/F-####.md`, `todo`, `tbd` | ✅ | Link to acceptance criteria |
...
```
.agentic/spec/adr/ADR-####-short-slug.md
```
...
✅ **MUST be valid**:
- `F-####` in FEATURES.md → must have `.agentic/spec/acceptance/F-####.md` (unless `todo` or `tbd`)
...
- `ADR-####` mentioned → must have file `.agentic/spec/adr/ADR-####-*.md`
...
**Run these regularly**:
```bash
bash .agentic/lib/tools/verify.sh  # Full check
python3 .agentic/lib/tools/doctor.py  # Quick health check
```
...
- Lessons/caveats:
  - See .agentic/spec/LESSONS.md#auth-timing-attacks
- Notes:
