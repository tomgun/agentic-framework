...
**Enable later**: `bash .agentic/lib/tools/enable-formal.sh`
...
## What you get
- **A repo init protocol** (agent-guided) that creates stable context artifacts: `STACK.md`, `CONTEXT_PACK.md`, `.agentic/STATUS.md`, `JOURNAL.md`, `spec/`, `.agentic/spec/adr/`.
...
- Fill in `STACK.md`, `.agentic/OVERVIEW.md`, `CONTEXT_PACK.md` (and `spec/` if Formal)
- Set up quality validation for your stack

The agent follows `.agentic/lib/init/init_playbook.md` which guides it through the entire initialization process.
...
1. **Run scaffold**: `bash .agentic/lib/init/scaffold.sh` (creates all template files)
...
4. **Fill in docs**: `STACK.md`, `.agentic/OVERVIEW.md`, `CONTEXT_PACK.md` (and `spec/` if Formal)
...
bash .agentic/lib/tools/upgrade.sh /path/to/your-project
```
...
- Current execution state: `.agentic/STATUS.md`
- Requirements: `spec/PRD.md`
- Architecture + testing strategy: `spec/TECH_SPEC.md`
- Feature/requirement registry (IDs + status + acceptance + test notes): `.agentic/spec/FEATURES.md`
- Acceptance criteria per feature: `.agentic/spec/acceptance/F-####.md`
- Lessons learned / caveats: `.agentic/spec/LESSONS.md` and `.agentic/spec/adr/*`
...
- `.agentic/STATUS.md`: current progress, next steps, known issues, roadmap.  
- `JOURNAL.md`: session-by-session progress log (new sessions, blockers, next steps).
- `.agentic/HUMAN_NEEDED.md`: items requiring human decision or intervention.
- `/spec/`: PRD + Tech Spec(s) + tasks (living docs).  
- `.agentic/spec/adr/`: Architecture Decision Records (only for real decisions).

## Tools and automation

30+ scripts in `.agentic/lib/tools/`. Key categories:
