...
## When the user wants to preview how to run the project

If the user says preview, how to run, dev server, start the app, run commands, what commands, or wants to know how to run/start/build/test the project:

**STOP.** Run `ag preview`. This detects the stack from STACK.md and auto-detection, then displays dev server, build, and test commands with source attribution (STACK.md vs auto-detected).
