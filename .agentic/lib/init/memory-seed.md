...
- **Capability catalog is universal**: FEATURES.md exists in ALL profiles. Discovery uses lightweight entries (`feature.sh cap add "Name" "Desc"`), formal uses full F-XXXX. Update it when building new capabilities — Claude hooks (Stop.sh, UserPromptSubmit) will nudge if you forget.
