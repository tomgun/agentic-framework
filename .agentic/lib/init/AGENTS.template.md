...
- When you identify something requiring human action (install dependency, make decision, access credentials), ADD IT TO `.agentic/HUMAN_NEEDED.md` IMMEDIATELY
...
- Keep `.agentic/OVERVIEW.md` up to date with vision and completed capabilities
- Keep `CONTEXT_PACK.md` current when architecture changes
- If this repo uses the Formal profile: keep `.agentic/STATUS.md` and `/spec/*` truthful
...
See `.agentic/lib/agents/shared/agent_operating_guidelines.md`
...
To regenerate: `bash .agentic/lib/tools/setup-agent.sh all`
