...
```bash
grep "^## F-" .agentic/spec/FEATURES.md | head -20
```
...
```bash
bash .agentic/lib/tools/doctor.sh       # Check project structure
...
grep -A 30 "^## F-0005:" .agentic/spec/FEATURES.md | grep "Code:"
```

### Find acceptance criteria for a feature
```bash
cat .agentic/spec/acceptance/F-0005.md
```

### Find decisions related to a topic
```bash
# Search ADR titles
ls .agentic/spec/adr/ | grep -i "auth"

# Search ADR content
grep -i "authentication" .agentic/spec/adr/*.md
```
...
grep -i "auth" .agentic/spec/LESSONS.md
```

### Check test coverage for a feature
```bash
grep -A 20 "^## F-0005:" .agentic/spec/FEATURES.md | grep -A 5 "^- Tests:"
```
...
| Needs human action (PR review, credentials, decision) | `.agentic/HUMAN_NEEDED.md` | `bash .agentic/lib/tools/blocker.sh add "Title" "type" "Details"` |
...
### Add a new feature
Edit `.agentic/spec/FEATURES.md` directly:
...
### Update priorities
Edit `.agentic/STATUS.md` - change "Next up" section with new order or new features.

### Mark a decision resolved
Edit `.agentic/HUMAN_NEEDED.md` - move item from "Active" to "Resolved" section.

### Note a new issue
Add to `.agentic/STATUS.md` under "Known issues / risks".

### Add acceptance criteria
Create or edit `.agentic/spec/acceptance/F-####.md` with specific criteria.

### Record a decision
Create `.agentic/spec/adr/ADR-####-short-title.md` from template.

### Add a reference
Add entry to `.agentic/spec/REFERENCES.md` for papers/docs you found useful.

### Update architecture notes
Edit `spec/TECH_SPEC.md` to add architectural details.

### Create a task
Run `bash .agentic/lib/tools/task.sh "Task title"` or create manually in `.agentic/spec/tasks/`.
...
# Verify docs updated
bash .agentic/lib/tools/verify.sh

# Check test coverage
bash .agentic/lib/tools/coverage.sh
```
...
| What features are done? | `grep "Status: shipped" .agentic/spec/FEATURES.md` |
| What's blocking progress? | `cat HUMAN_NEEDED.md` |
| Is documentation current? | `bash .agentic/lib/tools/verify.sh` |
...
2. **Alias common commands**: 
   ...
   alias acheck='bash .agentic/lib/tools/doctor.sh'
   ```
3. **Use grep with color**: `grep --color=always ...` makes patterns visible
4. **Pipe to less**: `bash .agentic/lib/tools/report.sh | less` for long output
5. **Save outputs**: `bash .agentic/lib/tools/feature_graph.sh --save` creates docs/feature_graph.md
...
echo ""
echo "▶ HEALTH CHECK"
bash .agentic/lib/tools/doctor.sh | grep -E "(OK|Missing|Validation)" | head -10
echo ""
echo "▶ FEATURES SUMMARY"
bash .agentic/lib/tools/report.sh | head -10
echo ""
