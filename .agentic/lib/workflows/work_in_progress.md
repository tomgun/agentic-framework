...
**Auto-generated by**: `.agentic/lib/tools/wip.sh`
...
bash .agentic/lib/tools/wip.sh start F-0005 "Implementing user authentication" \
...
bash .agentic/lib/tools/wip.sh checkpoint "Implemented login endpoint, starting JWT validation"
```
...
**UPDATED: `.agentic/lib/checklists/session_start.md`**
...
bash .agentic/lib/tools/wip.sh checkpoint "Login endpoint done, starting JWT"
...
**UPDATED: `.agentic/lib/claude-hooks/PreCompact.sh`**

```bash
# Before context compaction, update WIP checkpoint
if [[ -f ".agentic/session/WIP.md" ]]; then
  bash .agentic/lib/tools/wip.sh checkpoint "Context compaction triggered"
fi
```

**UPDATED: `.agentic/lib/claude-hooks/Stop.sh`**

```bash
# Warn user about uncommitted WIP
if [[ -f ".agentic/session/WIP.md" ]]; then
  echo "⚠️  .agentic/session/WIP.md exists - work may be incomplete!"
...
3. .agentic/session/WIP.md exists with last checkpoint: "Implementing JWT validation"
...
3. .agentic/session/WIP.md exists, SESSION_LOG.md has last checkpoint
...
3. User updates WIP: `bash .agentic/lib/tools/wip.sh checkpoint "Switching to Cursor"`
4. Opens Cursor
...
```bash
# Claude SessionStart reads .agentic/session/WIP.md
...
**Never commit while .agentic/session/WIP.md exists:**

Check in `before_commit.md`:
```bash
if [[ -f ".agentic/session/WIP.md" ]]; then
  echo "⚠️  .agentic/session/WIP.md still exists!"
  echo "   Complete work first: bash .agentic/lib/tools/wip.sh complete"
  exit 1
fi
```
...
```
[No .agentic/session/WIP.md] → bash wip.sh start → [.agentic/session/WIP.md: IN_PROGRESS]
                                         ↓
                                   (work + checkpoints)
                                         ↓
[.agentic/session/WIP.md: IN_PROGRESS] → bash wip.sh complete → [No .agentic/session/WIP.md] → git commit
                                         ↓
                              (if interrupted)
                                         ↓
[.agentic/session/WIP.md: STALE] → bash wip.sh check → Recovery prompt
                      ↓
...
bash .agentic/lib/tools/wip.sh checkpoint "Login endpoint done"
...
git add -A && git commit -m "feat: F-0005"
# .agentic/session/WIP.md still exists! (confusing for next session)
```
...
```bash
git reset --hard
bash .agentic/lib/tools/wip.sh complete  # Remove WIP lock
```
...
**Token cost:** Minimal (~50 tokens to create/update .agentic/session/WIP.md)
