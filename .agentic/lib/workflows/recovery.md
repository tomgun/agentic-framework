...
**Run at EVERY session start** (`.agentic/lib/checklists/session_start.md`):
...
2. Resume from checkpoint:
   ...
   - Update WIP checkpoint as you work:
     ```bash
     bash .agentic/lib/tools/wip.sh checkpoint "Resuming: implementing token validation"
     ```
...
4. Update WIP and continue:
   ```bash
   bash .agentic/lib/tools/wip.sh checkpoint "Fixed issues from interruption, continuing"
   ```
...
4. Clean WIP:
   ```bash
   bash .agentic/lib/tools/wip.sh complete  # Remove lock
   ```
...
5. Start fresh:
   ...
   - Create new WIP:
     ```bash
     bash .agentic/lib/tools/wip.sh start F-#### "description" "files"
     ```
...
bash .agentic/lib/tools/wip.sh checkpoint "Login endpoint complete"
bash .agentic/lib/tools/wip.sh checkpoint "JWT validation added"
bash .agentic/lib/tools/wip.sh checkpoint "Unit tests passing"
```

**Bad** ❌:
```bash
# Only at start, then nothing for 2 hours
bash .agentic/lib/tools/wip.sh start F-0005 "Auth"
...
- `.agentic/HUMAN_NEEDED.md` - Escalate ambiguous cases
