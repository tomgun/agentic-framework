...
### @feature F-####
- Links code to a feature in `.agentic/spec/FEATURES.md`
...
### @nfr NFR-#### (constraint)
- Links to non-functional requirement in `.agentic/spec/NFR.md`
...
1. Check `.agentic/spec/FEATURES.md` for the feature ID
2. Check `.agentic/spec/acceptance/F-####.md` for acceptance criteria IDs
3. Check `.agentic/spec/NFR.md` if the feature has NFR constraints
4. Add annotations to key functions/classes you create

### When editing existing code:
1. Check if code has `@feature` annotations
2. If editing changes feature behavior, update the relevant `.agentic/spec/FEATURES.md` entry
3. If editing impacts NFR compliance, verify NFR is still met
4. Update or add annotations as needed

### After implementing:
- Run `bash .agentic/lib/tools/coverage.sh` to check annotation coverage
- Update `.agentic/spec/FEATURES.md` with the "Code:" field pointing to annotated modules
...
- **Check coverage**: `python3 .agentic/lib/tools/coverage.py`
...
- **Trace**: `bash .agentic/lib/tools/ag.sh trace`
...
- **Verify consistency**: `bash .agentic/lib/tools/verify.sh`
