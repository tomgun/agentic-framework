...
bash .agentic/lib/tools/drift.sh      # Spec/code drift
bash .agentic/lib/tools/sync.sh --check # Staleness + drift detection
bash .agentic/lib/tools/version_check.sh # Version mismatches
```
...
- For detailed investigation: Enter **Research Mode** (see `.agentic/lib/workflows/research_mode.md`)
...
**Review these files:**
- `.agentic/spec/FEATURES.md`: Are features well-structured? Too many/too few?
- `.agentic/spec/NFR.md`: Are NFRs still relevant? Any missing?
- `spec/PRD.md`: Is product direction still clear?
- `spec/TECH_SPEC.md`: Is architecture documentation current?
- `.agentic/spec/acceptance/*.md`: Are acceptance criteria clear and testable?
...
**Run:**
```bash
bash .agentic/lib/tools/drift.sh --check  # Spec/code drift
```
...
- For deep investigation: Enter **Research Mode** (see `.agentic/lib/workflows/research_mode.md`)
...
3. Create tasks for action items (add to .agentic/spec/tasks/)
...
Agent reads `STACK.md` for settings and `.agentic/STATUS.md` for last retrospective date.
...
- Health checks: `.agentic/lib/tools/doctor.py`, `verify.sh`
- Scaling guidance: `.agentic/lib/workflows/scaling_guidance.md`
- Development loop: `.agentic/lib/workflows/dev_loop.md`, `tdd_mode.md`
