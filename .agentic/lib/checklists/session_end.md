...
- [ ] **`.agentic/OVERVIEW.md` updated** (optional - only if project has one)
...
- [ ] **Update `.agentic/HUMAN_NEEDED.md`** (if any blockers found)
...
- [ ] **Or verify `.agentic/HUMAN_NEEDED.md` is current** (if no new blockers)
...
- [ ] **Drift check** (recommended after significant work)
  - Run: `bash .agentic/lib/tools/drift.sh`
...
- [ ] **Suggest project health check**
  - `bash .agentic/lib/tools/doctor.sh` - if structure might have issues
  - `bash .agentic/lib/tools/verify.sh` - for comprehensive verification
...
- [ ] **Suggest feature status**
  - `bash .agentic/lib/tools/report.sh` - if working on features
  - `bash .agentic/lib/tools/feature_graph.sh` - if dependencies matter
...
```
**Suggested:** 
- Run `bash .agentic/lib/tools/doctor.sh` to verify project health
