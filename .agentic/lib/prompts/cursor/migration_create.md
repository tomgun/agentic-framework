...
1. **Create migration file:**
   ```bash
   bash .agentic/lib/tools/migration.sh create "[Concise title of change]"
   ```
   This will generate `.agentic/spec/migrations/XXX_[title].md` with the next ID.
...
3. **Update main spec files:**
   ...
   - `.agentic/spec/FEATURES.md` (if adding/modifying features)
   - `.agentic/spec/NFR.md` (if changing non-functional requirements)
