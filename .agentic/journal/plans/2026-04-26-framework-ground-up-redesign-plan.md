...
5. **The framework's own founding diagnosis**, from `docs/CAPABILITY_SPEC.md` line 9: *"~90% of the effort building this framework was compensating for unreliable agent behavior — not solving the actual development problems. Most of the complexity (behavioral tests, redundant instruction files, reinforcement layers, pre-commit gates) exists because agents kept not doing what they were told."* This is the strongest articulation of the diagnosis. CAPABILITY_SPEC.md enumerates 15 required capabilities (the **stable WHY**); `.agentic/spec/FEATURES.md` (v0.72.0, 529 lines, 42 features with YAML contract refs and consolidation maps) tracks the **current built WHAT** with ACs. **Critically, neither captures the wishlist** — what the user wanted but never made it into formal spec because the firefighting tax consumed the runway. The wishlist lives in TODO.md, CONTRIBUTIONS.md, proposed ADRs, JOURNAL future-tense entries, and is most consolidated in this plan's Pillar 4 / Theme A–J catalog (~70 items). The v5 redesign aims to deliver the 15 required capabilities at materially lower complexity *and* finally ship the wishlist that v0.7x couldn't.
