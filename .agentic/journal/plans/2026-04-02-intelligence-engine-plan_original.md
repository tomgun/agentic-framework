...
**Status**: DRAFT
**Feature**: F-TBD (Intelligence Engine)
**Date**: 2026-04-02
...
**Files**:
- `.agentic/intel/anatomy.yaml` — the file index (git-tracked, snapshot-based, regenerated by `ag intel scan`)
...
| `.agentic/intel/anatomy.yaml` | File intelligence database | git-tracked (regenerated by `ag intel scan`) |
...
### Phase 1: Foundation — anatomy + patterns + token ledger (~2 days)

1. Create `.agentic/intel/` directory
2. Implement `ag intel scan` — file walk, token estimation, summary extraction → anatomy.yaml
3. Implement `ag intel file PATH` — anatomy lookup
4. Implement `ag intel check PATH` — pattern match
5. Implement `ag intel learn` — pattern management
6. Implement token ledger (session + aggregation)
7. Implement `ag intel stats` — metrics display
8. Seed patterns.yaml from LESSONS.md
9. Wire into ag.sh dispatch
10. **Test**: scan produces valid anatomy, file/check/learn/stats all work

### Phase 2: Bootstrap — domain knowledge generation (~2 days)

1. Implement `ag intel bootstrap` — STACK.md → Claude generation → user review → write files
2. Generate: patterns, quality-checklist.yaml, test-strategy.yaml, NFR suggestions
3. Implement `ag intel retro` — analyze ISSUES.md + LESSONS.md → suggest updates
4. Integrate bootstrap as optional step in `ag init`
5. **Test**: bootstrap generates useful, domain-relevant content for sample stacks

### Phase 3: Phase-aware queries — intelligence assembly (~1 day)

1. Implement `ag intel architecture` — reads ADRs, NFRs, CONTEXT_PACK, quality-checklist.planning
2. Implement `ag intel spec F-XXXX` — reads FEATURES, contracts, NFR, quality-checklist.spec
...
5. Discovery-mode filtering (hide `[formal]` items when profile != formal)
6. **Test**: each query returns concise, relevant, phase-appropriate intelligence

### Phase 4: Hook integration — automatic for Claude Code (~1 day)

1. Extend PreToolUse.sh — Read anatomy hint + Write pattern check (pure bash, <100ms)
2. Extend PostToolUse.sh — Read/Write token tracking + async anatomy update
3. Extend stop.sh — token ledger finalization
4. Update hooks.json — add Read to PreToolUse matcher
5. **Prerequisite**: F-0300 hook registration
6. **Test**: Claude Code session gets automatic intelligence + token tracking

### Phase 5: Skill + instruction integration — all tools (~1 day)

1. Update 4 skills with `ag intel` references
2. Update 4 instruction file templates with trigger words
3. Update memory-seed
4. Extend Cursor context.sh with stats + phase intelligence
5. Update dashboard.sh to show token metrics
6. **Test**: skills naturally trigger intelligence queries, dashboard shows metrics
...
23. Quality checklist items are actionable, not generic platitudes
24. Token estimates within ~20% of actual (spot-check 10 files)
