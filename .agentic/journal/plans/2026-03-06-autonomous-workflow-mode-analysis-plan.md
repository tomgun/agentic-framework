# Analysis: Autonomous Claude Orchestrators — Lessons for Automatic Workflow Mode
...
### Project A (simplest — ~110 lines of bash)
...
### Project B (medium complexity — POSIX shell, ~2000 lines)

**Core mechanism**: Parses a `PLAN.md` into phases with dependencies, spawns fresh Claude CLI per phase, tracks progress in `PROGRESS.md`. Much more robust than Project A.
...
### Project C (most complex — Python backend + Electron desktop app)
...
|---------|------------------|-----------|
| Project A | Prompt engineering | Trust the agent to follow instructions |
| Project B | Process-level | Fresh instances + optional verification agent |
| Project C | SDK-level | Pre-tool-use hooks intercept every command |
...
### 2. Fresh Context Per Task (Project A + Project B pattern)
...
### 3. Verification by Separate Agent (Project B pattern)

Project B's `--verify` is elegant: after work is done, a SEPARATE read-only Claude instance reviews the changes, runs tests, checks the diff, and must output an explicit verdict. This "doer + verifier" separation is much more reliable than self-review.

### 4. Post-Processing in Code, Not in AI (Project C pattern)

Project C's `post_session_processing()` explicitly runs in Python, not Claude, because agent compliance is unreliable. State updates (progress tracking, recovery recording, memory saves) should be done by deterministic code after the agent session ends.

### 5. Structured Task Format (All three)

All three use machine-readable task definitions:
- Project A: `prd.json` with `passes: true/false`
- Project B: `PLAN.md` with `## Phase N:` format
- Project C: `implementation_plan.json` with subtask statuses

Our `spec/acceptance/F-XXXX.md` files could serve this role, but we'd need a machine-readable status tracking mechanism.

### 6. Escalation to Human (Project C pattern)

Project C has clear escalation rules: recurring issues (3+ times), consecutive errors (3+), max iterations (50). This prevents infinite loops while still allowing substantial autonomous work.
...
This is the simplest mode. Similar to Project B's `--verify` pattern:
...
Combines Project A's loop pattern with our spec-driven workflow:
...
1. **Loop runner** — spawns fresh Claude instances per iteration (like Project A)
2. **Task state tracker** — machine-readable progress per acceptance criterion
3. **Verification agent** — separate read-only Claude that checks work (like Project B)
4. **Post-iteration processor** — deterministic code that updates state (like Project C)
...
**None of them have what we have.** Project A is 110 lines of bash with zero enforcement. Project B is a phase runner with no spec system. Project C is the closest competitor but it's a monolithic desktop app — not a portable framework.
...
**What we're missing is the execution engine.** We tell agents WHAT to do and HOW to verify it, but we don't orchestrate WHEN to run them or manage the loop. That's what Project A, Project B, and Project C all provide.
...
Project A uses `prd.json` — simple `passes: true/false` per story. Project B uses plan phases. Project C uses `implementation_plan.json`.
...
Project A and Project B use `--dangerously-skip-permissions` and trust Claude completely. That's fine for solo prototyping. For a serious framework, we need layered safety:
...
- Based on project stack detection (similar to Project C's security profiles)
...
- Must output explicit `VERIFICATION_PASSED` keyword (Project B pattern)
...
This is a MASSIVE differentiator. Project C only works with Claude. Project B only works with Claude. Project A supports Amp + Claude but that's it.
...
| Dimension | Project A | Project B | Project C | Our Framework (Vision) |
|-----------|-------|------------|-------------|----------------------|
...
The magic: we're not building Project C's complexity. We're adding a **thin execution engine** (~500-1000 lines of shell) on top of our existing rich framework. The specs, gates, context system, and artifacts are already built. We just need the loop.
...
This maps to our D4 (Small Batch) perfectly. And it's exactly the pattern that makes Project A and Project B reliable.
...
## Previous learnings
{from .agentic/auto/F-XXXX-learnings.md — append-only, like Project A's progress.txt}
...
That's less than Project A (110 lines) plus Project B (2000 lines) combined, but with dramatically better safety, verification, and integration with a spec-driven workflow.

### Comparison: Our Architecture vs. The Three Repos

| Concern | Project A | Project B | Project C | Our Design |
|---------|-------|------------|-------------|------------|
...
Still dramatically simpler than Project C (~50K+ lines) while providing the full pipeline from research through verification.
...
This is the "Kanban board" view from Project C — but auto-generated from our structured state files.
