...
> **Review round 1**: Fresh adversarial review identified 12 issues (2 CRITICAL, 4 HIGH). Key fixes incorporated: reordered implementation sequence, acknowledged auto-commit complexity across ~15 files, recognized modes as a configuration spectrum not a taxonomy, identified F-0188 overlap, added kickoff validation via staging area. See "Reviewer Issues Addressed" section at the end.
>
> **Review round 2** (dialectical — Critic + Advocate in parallel with fresh context): Critic found 12 new issues (2 CRITICAL, 3 HIGH, 5 MEDIUM, 2 LOW). Advocate defended architecture decisions (modes-as-init-shortcuts, staging area, F-0188 decomposition). Key fixes: corrected F-0194/F-0191 shipped status claims, reframed auto-commit as explicit R2 amendment proposal (not "existing exception"), removed "low-dependency start" claim for F-0183, addressed F-0188→F-0187 dependency, added token budget for kickoff, fixed remaining A/B/C naming, removed fourth "Guided Autonomous" mode, added honest preview complexity. See "Round 2 Issues Addressed" section at the end.
...
| Plan-review loop | Shipped (F-0120) | `ag plan` |
| Dialectical plan review (Critic + Advocate) | In progress (F-0191, extends F-0120) | `ag plan` |
| Multi-agent coordination (AGENTS.json, agents_helpers.py) | Partial — agents_helpers.py works, worktree-by-default (F-0194) still planned | `agents_helpers.py` |
...
| Pre-commit quality gates (16 checks) | Shipped | `pre-commit-check.sh` |
...
**ADR-001 divergence note**: ADR-001 Section 4 proposed `subscribe_state` (event push model). The shipped coordination server uses `poll_changes` (pull model) instead. This is a deliberate simplification — polling is sufficient for the scheduler's current needs (seconds-level latency is acceptable for feature-level coordination). Event subscription may be needed later for real-time multi-agent scenarios but is not a gap for the current analysis.
...
**Mode 3 (Fully Autonomous)**: Execution engine exists (`ag auto epic`), but no way to get from prompt to the epic it needs. Also requires auto-commit/merge — a significant change touching ~15 files where "never auto-commit" is a constitutional principle (R2). This requires an explicit R2 amendment proposal (see Section 4.2 of the ADR). `task.py` currently auto-commits in automated execution — this is a known principle violation that the amendment would legitimize or that must be fixed.
...
**Severity: CRITICAL for Modes 2 and 3**
...
**Complexity note — be honest**: Even "simple" `npm run dev` involves port conflicts, process lifecycle, database dependencies, env vars, health checks, and cross-platform differences. MVP should start as **documentation** (detect stack → tell user how to run it) before attempting automation. Automated preview is a P2+ enhancement once the manual pattern proves valuable. Single-process stacks (no external DB) only for the first automated version.
...
**Current state**: `enable-formal.sh` creates formal directory structure. `migration.sh` handles spec contract changes for shipped features. Neither tool migrates discovery-phase *content* into formal spec structure — this is **new functionality**, not an enhancement to existing tools.

**What's needed**: A new `ag formalize` command (or extension to `enable-formal.sh`) that:
...
- Distinct from `migration.sh` (which handles shipped spec evolution) and `enable-formal.sh` (which creates directories only)
...
**Note on hybrid configurations**: Since modes are init shortcuts over a continuous spectrum (Section 2.1 of the ADR), users can configure hybrid behavior by overriding individual settings — e.g., `check_in_frequency: per_epic` with `feedback_mode: working_software` gives autonomous per-feature execution with human review at epic boundaries. No fourth named mode needed; the settings compose naturally.
...
2. **Gap 3: Taste & Style System** (F-0183, already spec'd, low-dependency start)
...
- **Gap 3** (F-0183) is already spec'd and a low-dependency start
...
The three modes (1/2/3) aren't three separate systems — they're the same pipeline with different **checkpoint configurations** and a **vision-to-backlog front-end**. The investment is mostly in:
...
### 3.1.1 Token Budget (F3 compliance)
  ...
  - Each interview phase should be a focused subagent with role-specific context (not one long session accumulating drift)
  - Generated artifacts (FEATURES.md entries, acceptance criteria stubs) should use structured formats (frontmatter, consistent field patterns) for downstream token efficiency
  - Kickoff should NOT load full codebase — CONTEXT_PACK.md + STACK.md + NFR.md is sufficient input context
...
### 4.2 Proposed Amendment to Principle R2
  ...
  - `task.py._commit_ac()` (lines 341-361) currently auto-commits without human review — this is a **known principle violation** in the shipped scheduler that should be either fixed or legitimized
  - Two distinct commit contexts exist in practice:
    - **Interactive agent sessions**: "never auto-commit" must remain absolute — agent shows diff, waits for human
    - **Automated execution** (scheduler, task runner): auto-commit may be allowed IF explicitly opted-in AND reviewed by critical agent
  - **Parent principle engagement**: R2 derives from D1 (Human-Agent Partnership) and D2 (Deterministic Enforcement). The amendment preserves D1 by requiring explicit opt-in (human chose to delegate) and preserves D2 by replacing behavioral hope with structural review (critical agent evaluates every diff). The human remains in control of the DECISION to delegate, even though they're not in the loop for individual commits.
  - **If this amendment is rejected**: `task.py._commit_ac()` must be fixed to require human approval in all contexts, which would make `ag auto epic` non-functional without a human watching every commit
...
**F-0188 dependency note**: F-0188 currently depends on F-0187 (Multi-Repo Umbrella) and F-0186. F-0187 is unshipped (planned). This dependency should be **relaxed**: end-to-end autonomous flow for SINGLE-REPO projects does not require multi-repo support. F-0187 becomes an enhancement, not a prerequisite. Update FEATURES.md to drop F-0187 from F-0188's hard dependencies.
...
1. **F-0183: Taste & Style System** — already spec'd, no dependencies. Note: "spec'd" ≠ quick — needs STACK.md style section, critical agent taste context loading, review prompt template, and wiring into review checkpoints. Estimated medium scope (~5-8 files). Positioned first because kickoff pipeline benefits from taste context being available.
...
Dialectical review (Critic + Advocate agents in parallel, fresh context). Critic found 12 issues; Advocate defended 8 architectural decisions. Synthesis resolved each finding:

| # | Severity | Critic Finding | Verdict | Resolution |
|---|----------|---------------|---------|------------|
| 1 | CRITICAL | F-0194 falsely claimed as shipped | VALID — FEATURES.md says planned | Fixed: corrected to "Partial — agents_helpers.py works, worktree-by-default still planned" |
| 2 | HIGH | F-0191 conflated with F-0120 | VALID — dialectical review is in_progress | Fixed: separated F-0120 (shipped) from F-0191 (in progress) |
| 3 | CRITICAL | R2 violation used as precedent instead of amendment | PARTIALLY VALID — Advocate's "two contexts" framing reasonable but Critic right about needing explicit amendment | Fixed: Section 4.2 reframed as explicit R2 amendment proposal with parent principle engagement (D1, D2) |
| 4 | MEDIUM | ADR-001 event model vs shipped polling divergence | VALID but low impact | Fixed: added divergence note acknowledging poll_changes vs subscribe_state, noting polling sufficient for MVP |
| 5 | MEDIUM | Mode A/B/C naming still present despite "fix" | VALID — lines 95 and 346 still had A/B/C | Fixed: all remaining instances standardized to 1/2/3 |
| 6 | HIGH | F-0183 "quick win" unjustified | PARTIALLY VALID — Advocate right it's a dependency, Critic right about scope | Fixed: removed "quick win", added honest scope estimate (~5-8 files), reframed as "low-dependency start" |
| 7 | HIGH | F-0188 depends on unshipped F-0187 | VALID — transitive dependency issue | Fixed: added note to relax F-0188 → F-0187 dependency (multi-repo is enhancement, not prerequisite for single-repo autonomy) |
| 8 | MEDIUM | Preview complexity dismissed | VALID | Fixed: MVP starts as documentation, automated preview is P2+ enhancement, single-process only |
| 9 | LOW | Fourth "Guided Autonomous" mode adds gratuitous scope | VALID per KISS | Fixed: removed named mode, noted settings compose naturally for hybrid configurations |
| 10 | MEDIUM | Gap 7 conflates migration.sh with new functionality | VALID | Fixed: clarified this is new `ag formalize` functionality, distinct from migration.sh and enable-formal.sh |
| 11 | MEDIUM | Token costs of kickoff pipeline ignored (violates F3) | VALID | Fixed: added Section 3.1.1 with token budgets (5-10K script, 15-25K playbook) and subagent architecture |
| 12 | LOW | File count contradicts itself (28+ vs ~15) | VALID | Fixed: standardized on ~15 with Section 4.4 as authoritative list |

**Advocate strengths confirmed**: Modes-as-init-shortcuts architecture, staging area pattern, F-0188 decomposition, deferral of user_expertise and feedback capture — all well-defended and preserved in revised plan.
