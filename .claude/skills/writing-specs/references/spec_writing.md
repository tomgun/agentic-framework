...
2. **Active NFR matching** — read `.agentic/spec/NFR.md` and evaluate EACH defined NFR:
   - For EACH NFR: does this feature's behavior intersect with the NFR's constraint?
...
4. **Create acceptance file** — `.agentic/spec/acceptance/F-XXXX.md` from `.agentic/spec/acceptance.template.md`
   ...
   - Add `### NFR Compliance` section with specific criteria per applicable NFR
...
1. Read `.agentic/spec/NFR.md` — evaluate EACH NFR for applicability to this feature
2. Add `NFRs:` field to FEATURES.md entry (list applicable NFR IDs, or `none` if checked and none apply)
3. Add `### NFR Compliance` section to acceptance criteria with specific measurable criteria per NFR
