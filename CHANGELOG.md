...
### Added
- **Claude Code statusline (R-101 follow-up)** — `ag hooks register` / `ag hooks install` now also wire a Claude Code statusline showing `project | branch | ctx N% | 5h N% reset HH:MM | wk N% reset Day HH:MM | task`. Powered by `.agentic/lib/tools/statusline.{py,sh}` reading the same token-ledger.jsonl that R-101 fills. The 5h/wk segments only render when `quota_pro_max_window_tokens` / `quota_pro_max_weekly_tokens` are set in STACK.md (template documents both, see `## Settings → Token quotas`). The installer JSON-merges into `.claude/settings.json` so existing permissions/mcpServers/customStatusLines are preserved; pass `--force-statusline` to overwrite. Honest limit: reset times are derived from the rolling-window edge of the local ledger, not the Anthropic dashboard's plan-anchored reset wall-clock.
- **Token Ledger Visible (R-101)** — `ag intel report --tokens` projects per-session + rolling-30 token usage from `token-ledger.jsonl`, broken down by tier/model/feature. Backed by a new Stop hook + SessionStart-recovery shim that emits one record per assistant turn (with feature attribution from gitBranch / AGENTS.json), so the ledger fills automatically as you work. The R-014 quota ring stays unchanged; the TUI header gains a second line showing `Session 287K • Roll 4.1M (12 sess) • Top F-006 612K` whenever data is present. `--json` for tooling. G5 golden-master fixture (`tests/fixtures/quota-fixture.jsonl` + `quota-fixture-expected.json`) guards M1 (cache-creation must NOT fold into `tokens_in`); 49 new tests across emitter / report / TUI.
- **Skills Marketplace (F-008 PR-A)** — `ag skills` CLI: `suggest|install|sync|list|remove|update-pins|request`. Installs community quality skills from a curated allowlist (`.agentic/lib/data/skills-marketplace.yaml`) with mandatory commit-sha pinning, raw-only HTTPS fetch from skills.sh, script quarantine (refuse-by-default unless `--accept-scripts`), and agent-agnostic fan-out to Claude + Cursor skill dirs via existing `generate-skills.sh` and `generate-cursor-skills.sh`. F-008 contract extended with AC-009/010/011 + M-002 migration entry. 14 unit tests cover allowlist schema, sha-pinning, script quarantine, scoped npm wildcard matching, path-traversal rejection, and upstream-repo issue routing. `ag init` integration ships in PR-B; STACK.md change-detection hook + instruction-file sync + LLM tests in PR-C.
- **MCP Task Delegation (F-018)** — 5 new coordination tools (`list_acs`, `get_task_brief`, `save_progress`, `get_next_action`, `get_delegation_prompt`) enable Claude Code sessions to delegate each AC to a fresh-context subagent via the native Agent tool — eliminating external CLI runner loops. MCP server auto-registered in `settings-template.json` via `ag auto init`. 123 tests (101 unit + 22 E2E over real stdio transport).
...
### Changed
- **Statusline now shows the active model + zero-config rate-limit cluster + repo-name in Docker** — `.agentic/lib/tools/statusline.py` reads rate-limit % and reset times directly from Claude Code's `rate_limits` envelope (Anthropic API rate-limit headers piped to the statusline command) instead of computing them from the local token-ledger gated on STACK.md ceilings. Works for every framework user out of the box — no `quota_pro_max_window_tokens` / `quota_pro_max_weekly_tokens` configuration required. Format groups all rate-limit info as a single bar segment: `N% 5h, M% 7d - reset HH:MM, Day HH:MM TZ (updated at main agent response)`. Pcts are listed first, reset times share one trailing TZ label (e.g. `EEST`, `PST`; falls back to `UTC±N`). The trailing parenthetical is ANSI-dimmed so it reads as a footnote rather than data; it anchors that the values are a snapshot from the last main-agent turn (`/usage` triggers a refresh). A new model segment renders the envelope's `display_name` (e.g. "Claude Opus 4.7"), falling back to `model.id` when display_name is absent. Project label now comes from `git config remote.origin.url` so it shows the actual repo name (e.g. `agentic-framework`) inside Docker containers where the toplevel basename is a mount point (e.g. `workspace`). Token ledger and STACK ceilings remain the source of truth for `ag intel report --quota` / `--tokens` analytics.
...
### Fixed
- **Backlog advancement now committed with feature completions** — The completing-work skill passively described backlog advancement but never called `backlog.sh done`. The committing-changes skill didn't mention staging state files. Both checklists in auto_orchestration.md lacked these steps. Now: explicit command in completing-work, staging reminder in committing-changes, checklist items in Feature Complete and Before Commit, and pre-commit sequence in memory-seed includes state file staging.
...
### Changed
- **Silent Session Start** — Agents no longer output preamble text ("Starting a new session...", "Let me check...") before the dashboard. The dashboard is the first text output. Enforced across all instruction layers: CLAUDE.md templates, session-start skill, checklist, auto_orchestration, memory-seed (v0.51.1), and Cursor/Claude prompt files. LLM test 001 updated with no-preamble assertion.
